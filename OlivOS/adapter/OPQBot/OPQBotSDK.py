    ...
    if target_event.sdk_event.payload.active:
        if target_event.sdk_event.payload.EventName == "CgiBaseResponse":
            ...
            if (
                ...
                and type(target_event.sdk_event.payload.EventData["MsgHead"]["FromUin"])
                is int
                and "ToUin" in target_event.sdk_event.payload.EventData["MsgHead"]
                and type(target_event.sdk_event.payload.EventData["MsgHead"]["ToUin"])
                is int
                ...
                and type(
                    target_event.sdk_event.payload.EventData["MsgHead"]["SenderNick"]
                )
                is str
                ...
                and type(target_event.sdk_event.payload.EventData["MsgBody"]["Content"])
                is str
            ):
                ...
                message_obj = OlivOS.messageAPI.Message_templet(
                    "olivos_string",
                    get_message(
                        Content=target_event.sdk_event.payload.EventData["MsgBody"][
                            "Content"
                        ],
                        AtUinLists=target_event.sdk_event.payload.EventData[
                            "MsgBody"
                        ].get("AtUinLists", []),
                    ),
                )
            ...
            if (
                ...
                and target_event.sdk_event.payload.EventData["MsgHead"]["MsgType"]
                == 528
                ...
                and type(target_event.sdk_event.payload.EventData["MsgHead"]["FromUid"])
                is str
            ):
                tmp_ToUin = str(
                    target_event.sdk_event.payload.EventData["MsgHead"]["ToUin"]
                )
                tmp_FromUin = str(
            ...
            if (
                ...
                and type(target_event.sdk_event.payload.EventData["Event"]["Invitee"])
                in [str, int]
                and "Invitor" in target_event.sdk_event.payload.EventData["Event"]
                and type(target_event.sdk_event.payload.EventData["Event"]["Invitor"])
                in [str, int]
            ):
                target_event.active = True
            ...
            False
            and target_event.sdk_event.payload.EventName
            == "ON_EVENT_FRIEND_SYSTEM_MSG_NOTIFY"
        ):
            if (
                type(target_event.sdk_event.payload.EventData) is dict
                ...
                and type(target_event.sdk_event.payload.EventData["MsgAdditional"])
                is str
            ):
                target_event.active = True
                target_event.plugin_info["func_type"] = "friend_add_request"
                ...
                target_event.data.flag = str(
                    target_event.sdk_event.payload.EventData["ReqUid"]
                )
        elif (
            target_event.sdk_event.payload.EventName
            == "ON_EVENT_GROUP_SYSTEM_MSG_NOTIFY"
        ):
            if (
                type(target_event.sdk_event.payload.EventData) is dict
                ...
                and type(target_event.sdk_event.payload.EventData["ActorUid"])
                in [str, int]
            ):
                if (
                    ...
                    target_event.data.flag = str(
                        target_event.sdk_event.payload.EventData["MsgSeq"]
                    )
                    gMsgSeqToGroupCodeReg[target_event.data.flag] = (
...
class PAYLOAD(object):
    ...
    class MessageSvc_PbSendMsg(payload_template):
        def __init__(
            self, ToUin: "int|str", ToType: int, Content: str, CurrentQQ: "int|str"
        ):
    ...
    class MessageSvc_PbSendMsg_all(payload_template):
        def __init__(
            self, ToUin: "int|str", ToType: int, dataPatch: dict, CurrentQQ: "int|str"
        ):
            payload_template.__init__(self)
            self.CgiCmd = "MessageSvc.PbSendMsg"
            self.CurrentQQ = str(CurrentQQ)
...
class event_action(object):
    ...
    def getUinfoCache(target_event, UidQuery, control_queue):
        ...
        this_msg = PAYLOAD.QueryUinByUid(
            UidQuery=UidQuery, CurrentQQ=target_event.base_info["self_id"]
        )
    ...
    def send_solo_all_msg(
        target_event, target_type, target_id, dataPatch, control_queue
    ):
        plugin_event_bot_hash = OlivOS.API.getBotHash(
            bot_id=target_event.base_info["self_id"],
            platform_sdk=target_event.platform["sdk"],
    ...
    def set_group_leave(target_event, group_id, control_queue):
        if target_event.bot_info != None:
            ...
            send_ws_event(
                plugin_event_bot_hash,
                PAYLOAD.exitGroup(
                    Uin=group_id, CurrentQQ=target_event.base_info["self_id"]
                ).dump(),
                control_queue,
            )
    ...
    def set_group_add_request(
        target_event, flag: str, sub_type: str, approve: bool, control_queue
    ):
        if target_event.bot_info != None:
            plugin_event_bot_hash = OlivOS.API.getBotHash(
                bot_id=target_event.base_info["self_id"],
            ...
            if raw_obj is not None:
                if (
                    ...
                    for raw_obj_this in raw_obj["GroupLists"]:
                        ...
                        tmp_res_data_this["id"] = init_api_do_mapping_for_dict(
                            raw_obj_this, ["GroupCode"], int
                        )
                        tmp_res_data_this["memo"] = ""
                        tmp_res_data_this["member_count"] = (
...
def send_ws_event(hash, data, control_queue):
    sendControlEventSend(
        "send",
        {
            "target": {"type": "OPQBot_link", "hash": hash},
