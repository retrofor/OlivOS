    ...
    def run(self):
        ...
        while True:
            try:
                asyncio.run(
                    start(
                        int(self.Proc_data["bot_info_dict"].post_info.access_token),
                        self,
                    )
                )
            except:
                self.log(
                    2, "OlivOS biliLive Link server [" + self.Proc_name + "] link lost"
                )
            time.sleep(5)
            ...
            while isLoop:
                try:
                    if "bili_jct" in cookie_new and "DedeUserID" in cookie_new:
                        ...
                        res = await OlivOS.biliLiveSDK.aiohttpGet(
                            session, OlivOS.biliLiveSDK.QRCODE_REQUEST_URL
                        )
                        ts = res["ts"]
                        ...
                        while True:
                            ...
                            if res["status"]:
                                isLoop = False
                                OlivOS.biliLiveSDK.save_cookies(
                                    cookie, conf_dir_path + "/cookies.json"
                                )
                                break
                            else:
                                code = res["data"]
                    ...
                    traceback.print_exc()
            Proc.log(
                2, "OlivOS biliLive Link server [" + Proc.Proc_name + "] login succeed"
            )
        bot = OlivOS.biliLiveSDK.BiliLiveBot(
            room_id=room, uid=0, session=session, loop=session._loop, Proc=Proc
        )
        await bot.init_room()
        bot.start()
                        ...
                        if "send" == rx_packet_data.key["data"]["action"]:
                            if "data" in rx_packet_data.key["data"]:
                                ...
                                if type(rx_packet_data.key["data"]["data"]) == dict:
                                    ...
                                    token = OlivOS.biliLiveSDK.get_cookies(
                                        cookie, "bili_jct"
                                    )
                                    if "msg" in tmp_data:
