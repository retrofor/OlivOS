...
class SDKHandler(OlivOS.thirdPartyModule.blivedm.BaseHandler):
    async def _on_danmaku(
        self,
        client: BiliLiveBot,
        message: OlivOS.thirdPartyModule.blivedm.models.DanmakuMessage,
    ):
        try:
...
def get_Event_from_SDK(target_event: event):
    ...
    if type_sdk_event == OlivOS.thirdPartyModule.blivedm.models.DanmakuMessage:
        sdk_payload: OlivOS.thirdPartyModule.blivedm.models.DanmakuMessage = (
            target_event.sdk_event.payload
        )
        target_event.active = True
        target_event.plugin_info["func_type"] = "group_message"
        message_obj = OlivOS.messageAPI.Message_templet(
            "olivos_string", sdk_payload.msg
        )
    ...
    def send_msg(target_event, message, control_queue):
        ...
        if len(message_new) > 0:
            send_ws_event(
                ...
                PAYLOAD.chat(message=message_new).data,
                control_queue,
            )
...
def send_ws_event(hash, data, control_queue):
    sendControlEventSend(
        "send",
        {
            "target": {"type": "biliLive_link", "hash": hash},
