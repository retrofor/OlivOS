...
class server(OlivOS.API.Proc_templet):
            ...
            try:
                ...
                if api_obj_json is not None:
                    ...
                    if "ticket" in api_obj_json:
                        self.Proc_data["extend_data"]["websocket_ticket"] = (
                            api_obj_json["ticket"]
                        )
                    else:
        ...
        try:
            ...
            if tmp_data_rx_obj.active:
                if tmp_data_rx_obj.data.type in [
                    ...
                    sdk_event = OlivOS.dingtalkSDK.event(
            ...
            if tmp_data_rx_obj.data.type == "SYSTEM":
                ...
                if tmp_data_rx_obj.data.topic == "disconnect":
                    self.log(
                        0,
                        "OlivOS dingtalk link server ["
                        + self.Proc_name
                        + "] websocket link will be closed by remote!",
                    )
    ...
    def on_error(self, ws, error):
        self.log(
            0,
            "OlivOS dingtalk link server [" + self.Proc_name + "] websocket link error",
        )

    def on_close(self, ws, close_status_code, close_msg):
        self.log(
            0,
            "OlivOS dingtalk link server [" + self.Proc_name + "] websocket link close",
        )

    def on_open(self, ws):
        self.log(
            2,
            "OlivOS dingtalk link server [" + self.Proc_name + "] websocket link start",
        )
    ...
    def run_websocket_rx_connect_start(self):
        ...
        self.log(
            2,
            "OlivOS dingtalk link server [" + self.Proc_name + "] websocket link lost",
        )
