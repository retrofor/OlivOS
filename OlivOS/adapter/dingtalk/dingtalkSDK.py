...
class bot_info_T(object):
    def __init__(self, id=-1, AppKey=None, AppSecret=None, model="private"):
        ...
        self._access_token = (
            None  # 在 dingtalk 中，access_token 7200s 过期，故需要基于函数生成
        )
    ...
    def access_token(self):
        ...
        if res is not None:
            ...
            self._expire_time = time_this + max(
                int(res["expireIn"]) - 200, 200
            )  # 提早 200s 更新 access token
            return self._access_token
        return None
...
class payload_template(object):
    ...
    def dump(self, res_code=None, message=None, data=None):
        res_obj = {
            "code": None,
            "headers": {"messageId": None, "contentType": "application/json"},
            "message": None,
            "data": None,
        }
        ...
        try:
            if headers is None:
                headers = {
                    ...
                    "x-acs-dingtalk-access-token": str(
                        self.bot_info.access_token
                    ),  # 此时会自动更新 access_token
                }
...
class API:
    ...
    class uploadMedia(api_templet):
        ...
        def do_api(self, req_type="POST", proxy=None):
            ...
            # )
            requset_args = {
                "params": {"access_token": str(self.bot_info.access_token)},
                "files": {
                    "media": (
                        self.data.mediaName,
                        self.data.media,
    ...
    class getUserInfo(api_templet):
        ...
        def do_api(self, req_type="POST", proxy=None):
            ...
            return self._do_api(req_type="POST", body=tmp_payload_dict)
    ...
    if target_event.sdk_event.payload.data.type in [
        ...
        if type(data_raw) == dict:
            ...
            if "atUsers" in data_raw:
                ...
                for at_user_this in data_raw["atUsers"]:
                    if "staffId" in at_user_this:
                        tmp_at_list.append(
                            OlivOS.messageAPI.PARA.at(at_user_this["staffId"])
                        )
                    elif at_user_this["dingtalkId"] == self_id_encrypted:
                        para_self_at = OlivOS.messageAPI.PARA.at(
            ...
            if data_raw["msgtype"] == "text":
                message_para_list.append(
                    OlivOS.messageAPI.PARA.text(data_raw["text"]["content"])
                )
                    ...
                    if "text" in para_this:
                        ...
                        if "type" in para_this:
                            if para_this["type"] == "picture":
                                ...
                                path = _download_file(
                                    target_event, download_code, ".png", "images"
                                )
                                message_para_list.append(
                                    OlivOS.messageAPI.PARA.image(path)
                                )
                if len(tmp_str) > 0:
                    message_para_list.append(
                        OlivOS.messageAPI.PARA.text("\n".join(tmp_str))
                ...
                path = _download_file(
                    target_event, download_code, f".{videoType}", "videos"
                )
        ...
        if message_obj.active:
            ...
            if data_raw["conversationType"] == "1":
                ...
                target_event.data = target_event.private_message(
                    str(sender_id), message_obj, "private"
                )
                target_event.plugin_info["func_type"] = "private_message"
                target_event.data.message_sdk = message_obj
                target_event.data.message_id = (
                    target_event.sdk_event.payload.data.messageId
                )
...
def _get_file_url(target_event, downloadCode):
    ...
    try:
        ...
        msg_api.set_data(msg_api.data_T(downloadCode=downloadCode))
        msg_api.do_api()
...
def _get_user_mobile(target_event, user_id: str):
    ...
    try:
        ...
        msg_api.set_data(msg_api.data_T(userid=user_id))
        msg_api.do_api()
        if msg_api.res is not None:
