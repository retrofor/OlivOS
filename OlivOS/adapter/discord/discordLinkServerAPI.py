...
class server(OlivOS.API.Proc_templet):
        ...
        try:
            ...
            if tmp_data_rx_obj.data.op == 0:
                if tmp_data_rx_obj.data.t in [
                    ...
                    sdk_event = OlivOS.discordSDK.event(
                        tmp_data_rx_obj, self.Proc_data["bot_info_dict"]
                    )
                    ...
                    self.log(
                        0,
                        "OlivOS discord link server ["
                        + self.Proc_name
                        + "] websocket identify ACK",
                    )
            elif tmp_data_rx_obj.data.op == 10:
                self.Proc_data["extend_data"]["pulse_interval"] = (
                    tmp_data_rx_obj.data.d["heartbeat_interval"] / 1000
                )
    ...
    def on_error(self, ws, error):
        self.log(
            0,
            "OlivOS discord link server [" + self.Proc_name + "] websocket link error",
        )

    def on_close(self, ws, close_status_code, close_msg):
        self.log(
            0,
            "OlivOS discord link server [" + self.Proc_name + "] websocket link close",
        )

    def on_open(self, ws):
        self.log(
            2,
            "OlivOS discord link server [" + self.Proc_name + "] websocket link start",
        )
    ...
    def run_pulse(self):
        ...
        self.log(
            0,
            "OlivOS discord link server [" + self.Proc_name + "] websocket pulse lost",
        )
        return
    ...
    def run_websocket_rx_connect_start(self):
        ...
        ws.run_forever(
            http_proxy_host=proxy_set[0],
            http_proxy_port=proxy_set[1],
            proxy_type=proxy_set[2],
        )
        ...
        self.log(
            ...
            "OlivOS discord link server ["
            + self.Proc_name
            + "] websocket link lost, will retry in 10s",
        )
