...
sdkAPIHost = {"default": "https://discord.com/api/v10"}
...
class PAYLOAD(object):
    ...
    class sendIdentify(payload_template):
        def __init__(
            ...
            bot_info: bot_info_T,
            intents=(
                int(intents_T.GUILDS)
                | int(intents_T.DIRECT_MESSAGE | intents_T.GUILD_MESSAGES)
            ),
        ):
            tmp_intents = intents
            ...
            try:
                ...
                if len(self.imagedata) > 0:
                    ...
                    payload, payload_content_type = encode_multipart_formdata(
                        fields=payload, boundary="boundary"
                    )
        ...
        if "content" in target_event.sdk_event.payload.data.d:
            if target_event.sdk_event.payload.data.d["content"] != "":
                message_obj = OlivOS.messageAPI.Message_templet(
                    "discord_string", target_event.sdk_event.payload.data.d["content"]
                )
    ...
    def send_msg(target_event, chat_id, message, flag_direct=False):
            ...
            if str(chat_id) in sdkDMInfo:
                ...
                if raw_obj is not None:
                    if type(raw_obj) == dict:
                        this_msg.metadata.channel_id = int(
        ...
        for message_this in message.data:
            ...
            if type(message_this) == OlivOS.messageAPI.PARA.image:
                ...
                this_msg.data.embeds.append({"description": res_this})
                this_msg.do_api()
