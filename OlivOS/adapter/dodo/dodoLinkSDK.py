    ...
    def do_api(self, req_type="POST"):
        try:
            ...
            headers = {
                ...
                % (str(self.bot_info.id), self.bot_info.access_token),
            }
...
class API(object):
    ...
    class setResourcePictureUpload(api_templet):
        ...
        def do_api(self, req_type="POST"):
            try:
                tmp_payload_dict = {
                    "file": (str(uuid.uuid4()) + ".png", self.data.file, "image/png")
                }
            ...
            if target_event.platform["model"] == "v1":
                ...
                if api_res_json["status"] == 0:
                    sdkSubSelfInfo[plugin_event_bot_hash] = str(
                        api_res_json["data"].get("dodoId", None)
                    )
            else:
                api_msg_obj.do_api("POST")
                api_res_json = json.loads(api_msg_obj.res)
                if api_res_json["status"] == 0:
                    sdkSubSelfInfo[plugin_event_bot_hash] = str(
                        api_res_json["data"].get("dodoSourceId", None)
                    )
        except:
            pass
    try:
        if target_event.sdk_event.payload.data.type == 0:
            if target_event.sdk_event.payload.data.data["eventType"] == str(2001):
                message_obj = None
                if (
                    target_event.sdk_event.payload.data.data["eventBody"]["messageType"]
                    == 1
                ):
                ...
                if message_obj is not None:
                    ...
                    tmp_host_id = str(
                        target_event.sdk_event.payload.data.data["eventBody"].get(
                            "islandSourceId", None
                        )
                    )
                    tmp_user_id = str(
                    ...
                    if target_event.platform["model"] == "v1":
                        if tmp_user_id not in sdkUserInfo[tmp_host_id]:
                            ...
                            if api_res_json["status"] == 0:
                                ...
                                sdkUserInfo[tmp_host_id][tmp_user_id] = api_res_json[
                                    "data"
                                ].copy()
                    else:
                        if tmp_user_id not in sdkUserInfo[tmp_host_id]:
                    ...
                    if tmp_user_id in sdkUserInfo[tmp_host_id]:
                        if "nickName" in sdkUserInfo[tmp_host_id][tmp_user_id]:
                            target_event.data.sender["nickname"] = sdkUserInfo[
                                tmp_host_id
                            ][tmp_user_id]["nickName"]
                            target_event.data.sender["name"] = sdkUserInfo[tmp_host_id][
                                tmp_user_id
                            ]["nickName"]
                ...
                if message_obj is not None:
                    ...
                    target_event.data = target_event.private_message(
                        str(tmp_user_id), message_obj, "private"
                    )
                    target_event.data.message_sdk = message_obj
                    target_event.data.message_id = str(
                        target_event.sdk_event.payload.data.data["eventBody"][
                    ...
                    if tmp_user_info:
                        if "nickName" in tmp_user_info:
                            target_event.data.sender["nickname"] = tmp_user_info[
                                "nickName"
                            ]
    ...
    def send_msg(target_event: OlivOS.API.Event, chat_id, message):
        ...
        for message_this in message.data:
            if type(message_this) is OlivOS.messageAPI.PARA.text:
                ...
                this_msg.data.messageBody = event_action.setImageUploadFast(
                    target_event, message_this.data["file"]
                )
            ...
            if raw_obj is not None:
                if type(raw_obj) == dict:
                    ...
                    res_data["data"]["id"] = init_api_do_mapping_for_dict(
                        raw_obj, ["data", "dodoSourceId"], str
                    )
                    if res_data["data"]["id"] is None:
                        res_data["data"]["id"] = init_api_do_mapping_for_dict(
                            raw_obj, ["data", "dodoId"], str
                        )
        except:
