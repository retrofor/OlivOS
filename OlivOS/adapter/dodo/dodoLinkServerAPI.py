...
class server(OlivOS.API.Proc_templet):
        ...
        try:
            ...
            if tmp_data_rx_obj.active:
                if tmp_data_rx_obj.data.type == 0:
                    sdk_event = OlivOS.dodoLinkSDK.event(
    ...
    def on_error(self, ws, error):
        self.log(
            0, "OlivOS dodo link server [" + self.Proc_name + "] websocket link error"
        )

    def on_close(self, ws, close_status_code, close_msg):
        self.log(
            0, "OlivOS dodo link server [" + self.Proc_name + "] websocket link close"
        )

    def on_open(self, ws):
        self.log(
            2, "OlivOS dodo link server [" + self.Proc_name + "] websocket link start"
        )
    ...
    def run_websocket_rx_connect_start(self):
        ...
        self.log(
            2, "OlivOS dodo link server [" + self.Proc_name + "] websocket link lost"
        )
