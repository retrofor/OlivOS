    ...
    def run_poll_list(self):
        for bot_info_this in self.Proc_data["bot_info_dict"]:
            ...
            if bot_info_this_obj.platform["sdk"] == "dodo_poll":
                sdk_bot_info_this = (
                    OlivOS.dodoSDK.get_SDK_bot_info_from_Plugin_bot_info(
                        bot_info_this_obj
                    )
                )
                ...
                if bot_info_this not in self.Proc_data["bot_info_token_life"]:
                    ...
                    self.Proc_data["bot_info_token_life"][bot_info_this] = (
                        self.Proc_data["bot_info_token_life_counter"]
                    )
                else:
                            ...
                            if res_obj["status"] == 0:
                                if type(res_obj["data"]) == dict:
                                    if type(res_obj["data"]["islands"]) == list:
                                        for tmp_islands_this in res_obj["data"][
                                            "islands"
                                        ]:
                ...
                for tmp_island_list_this in self.Proc_data["bot_info_island_list"][
                    ...
                    sdk_api_tmp_2 = OlivOS.dodoSDK.API.getIslandUpdate(
                        sdk_bot_info_this
                    )
                    ...
                    if not flag_not_attach_2:
                        try:
                            ...
                            tmp_message_id_max = self.Proc_data["bot_info_update_id"][
                                bot_info_this
                            ]
                            if res_obj_2["status"] == 0:
                                if type(res_obj_2["data"]) == dict:
                                    if type(res_obj_2["data"]["messages"]) == list:
                                        for tmp_messages_this in res_obj_2["data"][
                                            "messages"
                                        ]:
                                            if (
                                                self.Proc_data["bot_info_update_id"][
                                                ...
                                                if (
                                                    tmp_messages_this["uid"]
                                                    != bot_info_this_obj.id
                                                ):
                                                    ...
                                                    tx_packet_data = OlivOS.pluginAPI.shallow.rx_packet(
                                                        sdk_event
                                                    )
                                                    self.Proc_info.tx_queue.put(
                                                        tx_packet_data, block=False
                                                    )
                                            if (
                                                tmp_message_id_max
                                                < tmp_messages_this["id"]
                                            ):
                                                tmp_message_id_max = tmp_messages_this[
                                                    "id"
                                                ]
                                        self.Proc_data["bot_info_update_id"][
                                            bot_info_this
