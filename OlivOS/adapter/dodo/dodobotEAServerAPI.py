...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        while True:
            ...
            msg_res = req.request(
                "GET",
                OlivOS.dodobotEASDK.post_host
                + ":"
                + str(OlivOS.dodobotEASDK.post_port)
                + "/GetAccounts",
                headers=headers,
                data="",
            )
        ...
        while True:
            try:
                async with websockets.connect(
                    OlivOS.dodobotEASDK.websocket_host
                    + ":"
                    + str(OlivOS.dodobotEASDK.websocket_port)
                ) as websocket:
                    while True:
                        tmp_recv_pkg = None
                        ...
                        try:
                            ...
                            tmp_recv_pkg = None
                        if tmp_recv_pkg is not None and type(tmp_recv_pkg) == dict:
                            if "Data" in tmp_recv_pkg:
                                if type(tmp_recv_pkg["Data"]) == str:
                                    try:
                                        tmp_recv_pkg_data = json.loads(
                                            tmp_recv_pkg["Data"]
                                        )
                                    except:
                                        tmp_recv_pkg_data = None
                        if tmp_recv_pkg_data is not None:
                            for bot_info_this in self.Proc_data["bot_info_dict"]:
                                bot_info_this_obj = self.Proc_data["bot_info_dict"][
                                    bot_info_this
                                ]
                                if (
                                    ...
                                    sdk_event = OlivOS.dodobotEASDK.event(
                                        tmp_recv_pkg_data, sdk_bot_info_this
                                    )
