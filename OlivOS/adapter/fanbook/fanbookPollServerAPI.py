    ...
    def run_poll_list(self, flag_first):
        for bot_info_this in self.Proc_data["bot_info_dict"]:
            ...
            if bot_info_this_obj.platform["sdk"] == "fanbook_poll":
                sdk_bot_info_this = (
                    OlivOS.fanbookSDK.get_SDK_bot_info_from_Plugin_bot_info(
                        bot_info_this_obj
                    )
                )
                flag_not_attach = False
                ...
                try:
                    ...
                    self.log(3, skip_result, [("fanbookPollServer", "default")])
                            ...
                            if res_obj["ok"] is True:
                                if type(res_obj["result"]) == list:
                                    for tmp_messages_this in res_obj["result"]:
                                        sdk_event = OlivOS.fanbookSDK.event(
                                            tmp_messages_this, bot_info_this_obj
                                        )
                                        tx_packet_data = (
                                            OlivOS.pluginAPI.shallow.rx_packet(
                                                sdk_event
                                            )
                                        )
