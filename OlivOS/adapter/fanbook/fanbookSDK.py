...
fanbookAPIHost = {"a1": "https://a1.fanbook.mobi"}

fanbookAPIRoute = {"apiroot": "/api/bot"}
...
class api_templet(object):
    ...
    def do_api(self):
        if True:
            ...
            tmp_fanbookAPIRouteTemp.update({"token": self.bot_info.access_token})
...
def get_Event_from_SDK(target_event):
    ...
    if checkByListAnd([
        ...
        target_event.data.message_id = str(
            target_event.sdk_event.json["channel_post"]["message_id"]
        )
        ...
        target_event.data.sender["name"] = target_event.sdk_event.json["channel_post"][
            "from"
        ]["first_name"]
        ...
        try:
            ...
            if tmp_res_obj["ok"]:
                ...
                res_data["data"]["name"] = init_api_do_mapping_for_dict(
                    tmp_res_obj, ["result", "username"], str
                )
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(tmp_res_obj, ["result", "id"], int)
                )
        except:
