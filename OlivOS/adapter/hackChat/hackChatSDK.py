        ...
        if target_event.sdk_event.payload.cmd == "onlineSet":
            ...
            if (
                ...
                if (
                    ...
                    target_event.data = target_event.group_message_sent(
                        str(0), str(user_hash), message_obj, "group"
                    )
                else:
                ...
                target_event.data.sender["nickname"] = (
                    target_event.sdk_event.payload.data["nick"]
                )
                target_event.data.sender["id"] = str(user_hash)
                target_event.data.sender["name"] = target_event.sdk_event.payload.data[
                    "nick"
                ]
            ...
            if True:
                ...
                target_event.data = target_event.group_member_increase(
                    str(0), str(0), str(user_hash), "approve"
                )
            ...
            if True:
                ...
                target_event.data = target_event.group_member_decrease(
                    str(0), str(0), str(user_hash), "leave"
                )
    ...
    def send_msg(target_event, message, control_queue):
        ...
        if len(message_new) > 0:
            send_ws_event(
                ...
                PAYLOAD.chat(message=message_new).dump(),
                control_queue,
            )
...
def send_ws_event(hash, data, control_queue):
    sendControlEventSend(
        "send",
        {
            "target": {"type": "hackChat_link", "hash": hash},
