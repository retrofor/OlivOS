...
def get_SDK_bot_info_from_Event(target_event):
    res = bot_info_T(
        target_event.bot_info.id, target_event.bot_info.post_info.access_token
    )
...
class API(object):
    ...
    class getUserView(api_templet):
        def __init__(self, bot_info=None):
            ...
            self.host = sdkAPIHost["default"]
            self.route = (
                sdkAPIRoute["user"] + "/view?user_id={user_id}&guild_id={guild_id}"
            )
    ...
    if "type" in target_event.sdk_event.payload.data.d:
        if (
            ...
            message_obj = OlivOS.messageAPI.Message_templet(
                "kaiheila_string", target_event.sdk_event.payload.data.d["content"]
            )
            ...
            message_obj = OlivOS.messageAPI.Message_templet(
                ...
                [
                    OlivOS.messageAPI.PARA.image(
                        file=target_event.sdk_event.payload.data.d["content"]
                    )
                ],
            )
        elif (
                ...
                try:
                    if (
                        ...
                        target_event.data.message_id = str(
                            target_event.sdk_event.payload.data.d["msg_id"]
                        )
                        ...
                        target_event.data.host_id = str(
                            target_event.sdk_event.payload.data.d["extra"]["guild_id"]
                        )
                        ...
                        if str(target_event.data.user_id) == str(
            ...
            if message_obj.active:
                ...
                if (
                    ...
                    if (
                        ...
                        and type(target_event.sdk_event.payload.data.d["extra"]["body"])
                        is dict
                        and "type" in target_event.sdk_event.payload.data.d["extra"]
                        and target_event.sdk_event.payload.data.d["extra"]["type"]
                        == "message_btn_click"
                    ):
                        if (
                            "channel_type"
                            in target_event.sdk_event.payload.data.d["extra"]["body"]
                            and target_event.sdk_event.payload.data.d["extra"]["body"][
                                "channel_type"
                            ]
                            == "PERSON"
                            and "target_id"
                            in target_event.sdk_event.payload.data.d["extra"]["body"]
                            and type(
                                target_event.sdk_event.payload.data.d["extra"]["body"][
                                    "target_id"
                                ]
                            )
                            is str
                            ...
                            target_event.active = True
                            target_event.plugin_info["func_type"] = "private_message"
                            msg = target_event.sdk_event.payload.data.d["extra"][
                                "body"
                            ]["value"]
                            user_id = target_event.sdk_event.payload.data.d["extra"][
                                "body"
                            ]["user_info"]["id"]
                            user_name = target_event.sdk_event.payload.data.d["extra"][
                                "body"
                            ]["user_info"]["username"]
                            message_obj = OlivOS.messageAPI.Message_templet(
                                "olivos_para", [OlivOS.messageAPI.PARA.text(text=msg)]
                            )
                            target_event.data = target_event.private_message(
                                user_id, message_obj, "friend"
                            )
                            target_event.data.message_sdk = message_obj
                            ...
                            host_id = target_event.sdk_event.payload.data.d["extra"][
                                "body"
                            ]["guild_id"]
                            group_id = target_event.sdk_event.payload.data.d["extra"][
                                "body"
                            ]["target_id"]
                            user_id = target_event.sdk_event.payload.data.d["extra"][
                                "body"
                            ...
                            target_event.data = target_event.group_message(
                                group_id, user_id, message_obj, "group"
                            )
                            target_event.data.message_sdk = message_obj
                            target_event.data.message_id = str(-1)
                            target_event.data.raw_message = message_obj
...
class event_action(object):
    def send_msg(
        target_event, chat_id, message, flag_direct=False, message_type_in="card"
    ):
    ...
    def create_message(
        target_event, chat_id, content_type: int, content: str, flag_direct=False
    ):
            ...
            if raw_obj is not None:
                if type(raw_obj) == dict:
                    ...
                    res_data["data"]["name"] = init_api_do_mapping_for_dict(
                        raw_obj, ["data", "username"], str
                    )
                    res_data["data"]["id"] = init_api_do_mapping_for_dict(
                        raw_obj, ["data", "id"], str
                    )
        except:
        ...
        try:
            ...
            if raw_obj is not None:
                if type(raw_obj) == dict:
                    ...
                    res_data["data"]["times"]["join_time"] = (
                        init_api_do_mapping_for_dict(
                            raw_obj, ["data", "joined_at"], int
                        )
                    )
    ...
    def setResourceUploadFast(target_event, url: str, type_path: str = "images"):
        ...
        try:
            ...
            msg_upload_api = API.setResourcePictureUpload(
    ...
    def set_playgame_delete_activity(target_event, data_type: int):
        raw_obj = None
        res_data = OlivOS.contentAPI.api_result_data_template.universal_result()
        this_msg = API.setPlayGameDeleteActivity(
            get_SDK_bot_info_from_Event(target_event)
        )
...
class inde_interface(OlivOS.API.inde_interface_T):
    ...
    def __set_playgame_delete_activity_all(target_event, flag_log=True):
        res_data = None
        res_data = OlivOS.kaiheilaSDK.event_action.set_playgame_delete_activity_all(
            target_event
        )
        return res_data

    def set_playgame_delete_activity_all(
        self, flag_log: bool = True, remote: bool = False
    ):
        res_data = None
        if remote:
            pass
        else:
            res_data = inde_interface.__set_playgame_delete_activity_all(
                self.event, flag_log=True
            )
        return res_data
    ...
    def __set_playgame_activity_game(target_event, game_id, flag_log=True):
        res_data = None
        res_data = OlivOS.kaiheilaSDK.event_action.set_playgame_activity_game(
            target_event, game_id
        )
        return res_data

    def set_playgame_activity_game(
        self, game_id: int, flag_log: bool = True, remote: bool = False
    ):
        res_data = None
        if remote:
            pass
        else:
            res_data = inde_interface.__set_playgame_activity_game(
                self.event, game_id, flag_log=True
            )
        return res_data

    @OlivOS.API.Event.callbackLogger(
        "kaiheila:create_message", ["chat_type", "chat_id", "content_type", "content"]
    )
    ...
    def create_message(
        ...
        if remote:
            ...
            res_data = inde_interface.__create_message(
                self.event, chat_type, chat_id, content_type, content, flag_log=True
            )
        return res_data

    @OlivOS.API.Event.callbackLogger(
        "kaiheila:set_playgame_activity_music", ["music_name", "singer", "software"]
    )
    def __set_playgame_activity_music(
        target_event, music_name, singer, software, flag_log=True
    ):
        res_data = None
    ...
    def set_playgame_activity_music(
        ...
        if remote:
            ...
            res_data = inde_interface.__set_playgame_activity_music(
                self.event, music_name, singer, software, flag_log=True
            )
        return res_data
