...
class server(OlivOS.API.Proc_templet):
        ...
        while True:
            ...
            try:
                ...
                if api_obj_json["retcode"] == 0:
                    self.Proc_data["extend_data"]["ws_PLogin"]["uid"] = int(
                        api_obj_json["data"]["uid"]
                    )
                    ...
                    self.Proc_data["extend_data"]["ws_PLogin"]["platform"] = int(
                        api_obj_json["data"]["platform"]
                    )
                    self.Proc_data["extend_data"]["ws_PLogin"]["app_id"] = int(
                        api_obj_json["data"]["app_id"]
                    )
                    self.Proc_data["extend_data"]["ws_PLogin"]["device_id"] = str(
                        api_obj_json["data"]["device_id"]
                    )
                    self.Proc_data["extend_data"]["websocket_url"] = api_obj_json[
                        "data"
                    ]["websocket_url"]
                else:
    ...
    def on_error(self, ws: websocket.WebSocketApp, error):
        self.log(
            0,
            "OlivOS mhyVila link server [" + self.Proc_name + "] websocket link error",
        )

    def on_close(self, ws: websocket.WebSocketApp, close_status_code, close_msg):
        self.log(
            0,
            "OlivOS mhyVila link server [" + self.Proc_name + "] websocket link close",
        )
    ...
    def on_open(self, ws: websocket.WebSocketApp):
        ...
        self.log(
            2,
            "OlivOS mhyVila link server [" + self.Proc_name + "] websocket link start",
        )
    ...
    def run_pulse(self):
        ...
        while self.Proc_data["extend_data"]["pulse_interval"] is not None:
            ...
            if (
                ...
                self.log(
                    0,
                    "OlivOS mhyVila link server ["
                    + self.Proc_name
                    + "] websocket pulse giveup",
                )
                return
            if self.Proc_data["extend_data"]["ws_obj"] is not None:
                try:
                    self.Proc_data["extend_data"]["ws_obj"].send(
                        tmp_data, opcode=websocket.ABNF.OPCODE_BINARY
                    )
                    self.log(
                        0,
                        "OlivOS mhyVila link server ["
                        + self.Proc_name
                        + "] websocket pulse send",
                    )
                except:
                    break
            else:
        ...
        self.log(
            0,
            "OlivOS mhyVila link server [" + self.Proc_name + "] websocket pulse lost",
        )
        return
    ...
    def send_PLogin(self, ws: websocket.WebSocketApp):
        ...
        tmp_data.data.device_id = self.Proc_data["extend_data"]["ws_PLogin"][
            "device_id"
        ]
        tmp_data.dump()
        # print(tmp_data)
    ...
    def run_websocket_rx_connect_start(self):
        ...
        self.log(
            2, "OlivOS mhyVila link server [" + self.Proc_name + "] websocket link lost"
        )
