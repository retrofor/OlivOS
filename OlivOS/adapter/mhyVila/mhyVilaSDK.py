...
sdkAPIHost = {"default": "https://bbs-api.miyoushe.com"}

sdkAPIRoute = {"platform": "/vila/api/bot/platform"}
                ...
                if (
                    ...
                    if s_type == "mentioned_robot":
                        ...
                        if type(s_bot_id) is str:
                            ...
                            set_name(
                                s_bot_id, msg[i_offset + 1 : i_offset + i_length - 1]
                            )
                    ...
                    if s_type == "mentioned_user":
                        ...
                        if type(s_user_id) is str:
                            ...
                            set_name(
                                s_user_id, msg[i_offset + 1 : i_offset + i_length - 1]
                            )
                            count = i_offset + i_length
        if count <= len(msg):
            res_msg += msg[count:]
...
def release_message(msg: str):
    res = {"content": {"text": "", "entities": []}}
    message_obj = OlivOS.messageAPI.Message_templet("olivos_string", msg)
    for message_obj_this in message_obj.data:
        if type(message_obj_this) is OlivOS.messageAPI.PARA.at:
            ...
            obj_this = {"offset": i_offset, "length": i_length}
...
def get_Event_from_SDK(target_event):
    ...
    if (
        ...
        messageData = target_event.sdk_event.payload.get("extendData", {}).get(
            "sendMessage", None
        )
        if type(messageData) is dict:
            ...
            if (
                ...
                if (
                    ...
                    target_event.data.sender["nickname"] = target_event.data.sender[
                        "name"
                    ]
...
class payload_template(object):
    ...
    class dataHeader_T(object):
        def __init__(self):
            ...
            self.BizType: int = (
                0  # uint32    消息体的业务类型，用于标识Body字段中的消息所属业务类型
            )
    ...
    def __dump_magicHeader(self):
        return struct.pack("<II", self.magicHeader.Magic, self.magicHeader.DataLen)
    ...
    def dump(self):
        ...
        self.raw = (
            self.__dump_magicHeader() + self.__dump_dataHeader() + self.dataDataBody
        )
        return self.raw

    def load(self):
        global recvProtoDict
        if self.raw is not None and len(self.raw) >= 32:
            self.magicHeader.Magic, self.magicHeader.DataLen = struct.unpack(
                "<II", self.raw[:8]
            )
            (
                self.dataHeader.HeaderLen,
                ...
                self.dataHeader.BizType,
                self.dataHeader.AppId,
            ) = struct.unpack("<IQIIi", self.raw[8:32])
...
class event_action(object):
    ...
    def create_panel_message(
        target_event, chat_id, object_name, content: dict, host_id=None
    ):
...
class inde_interface(OlivOS.API.inde_interface_T):
    @OlivOS.API.Event.callbackLogger(
        "mhyVila:create_message", ["chat_type", "chat_id", "object_name", "content"]
    )
