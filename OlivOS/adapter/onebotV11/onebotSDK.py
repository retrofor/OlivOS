        ...
        if (
            ...
            try:
                ...
                if len(self.bot_info.access_token) > 0:
                    ...
                    token_dict = {
                        "Authorization": f"Bearer {self.bot_info.access_token}"
                    }
                ...
                if self.bot_info.debug_mode:
                    if self.bot_info.debug_logger is not None:
                        self.bot_info.debug_logger.log(
                            0, self.node_ext + " - sendding succeed: " + msg_res.text
                        )
...
def get_Event_from_SDK(target_event):
    ...
    target_event.base_info["time"] = target_event.sdk_event.base_info.get(
        "time", int(time.time())
    )
    ...
    if target_event.base_info["type"] == "message_sent":
        ...
        if target_event.sdk_event.json["message_type"] == "private":
            ...
            target_event.data.message_id = str(
                target_event.sdk_event.json["message_id"]
            )
            ...
            if "user_id" in target_event.sdk_event.json["sender"]:
                target_event.data.sender["id"] = str(
                    target_event.sdk_event.json["sender"]["user_id"]
                )
        ...
        if target_event.sdk_event.json["message_type"] == "private":
            ...
            if target_event.sdk_event.json["sub_type"] == "channel":
                ...
                if "guild_id" in target_event.sdk_event.json:
                    target_event.data.host_id = str(
                        target_event.sdk_event.json["guild_id"]
                    )
                ...
                if "self_tiny_id" in target_event.sdk_event.json:
                    target_event.data.extend["sub_self_id"] = str(
                        target_event.sdk_event.json["self_tiny_id"]
                    )
                    if str(target_event.sdk_event.json["user_id"]) == str(
                        target_event.sdk_event.json["self_tiny_id"]
                    ):
        ...
        if target_event.sdk_event.json["notice_type"] == "group_upload":
            ...
            if target_event.sdk_event.json["sub_type"] == "poke":
                ...
                if "group_id" in target_event.sdk_event.json:
                    target_event.data.group_id = str(
                        target_event.sdk_event.json["group_id"]
                    )
...
class event_action(object):
    ...
    def get_msg(target_event, message_id):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                ...
                res_data["data"]["message_id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["message_id"], int)
                )
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["real_id"], int)
                )
                res_data["data"]["sender"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["sender", "user_id"], int)
                )
                res_data["data"]["sender"]["name"] = init_api_do_mapping_for_dict(
                ...
                res_data["data"]["time"] = init_api_do_mapping_for_dict(
                    raw_obj, ["time"], int
                )
                res_data["data"]["message"] = init_api_do_mapping_for_dict(
                    raw_obj, ["message"], str
                )
                res_data["data"]["raw_message"] = init_api_do_mapping_for_dict(
                    raw_obj, ["raw_message"], str
                )
        return res_data
    ...
    def set_group_anonymous_ban(
        target_event, group_id, anonymous, anonymous_flag, duration=1800
    ):
        this_msg = api.set_group_anonymous_ban(
    ...
    def set_group_special_title(
        target_event, group_id, user_id, special_title, duration
    ):
        this_msg = api.set_group_special_title(
            get_SDK_bot_info_from_Event(target_event)
        )
        this_msg.data.group_id = int(group_id)
        this_msg.data.user_id = int(user_id)
    ...
    def get_login_info(target_event):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                res_data["active"] = True
                res_data["data"]["name"] = init_api_do_mapping_for_dict(
                    raw_obj, ["nickname"], str
                )
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["user_id"], int)
                )
        return res_data
        ...
        if raw_obj is not None:
            if type(raw_obj) == list:
                ...
                for raw_obj_this in raw_obj:
                    ...
                    tmp_res_data_this["id"] = str(
                        init_api_do_mapping_for_dict(raw_obj_this, ["user_id"], int)
                    )
    ...
    def get_group_info(target_event, group_id, no_cache=False):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                res_data["active"] = True
                res_data["data"]["name"] = init_api_do_mapping_for_dict(
                    raw_obj, ["group_name"], str
                )
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["group_id"], int)
                )
                res_data["data"]["memo"] = init_api_do_mapping_for_dict(
                    raw_obj, ["group_memo"], str
                )
                res_data["data"]["member_count"] = init_api_do_mapping_for_dict(
                    raw_obj, ["member_count"], int
                )
                res_data["data"]["max_member_count"] = init_api_do_mapping_for_dict(
                    raw_obj, ["max_member_count"], int
                )
        return res_data
        ...
        if raw_obj is not None:
            if type(raw_obj) == list:
                ...
                for raw_obj_this in raw_obj:
                    ...
                    tmp_res_data_this["id"] = str(
                        init_api_do_mapping_for_dict(raw_obj_this, ["group_id"], int)
                    )
                    tmp_res_data_this["memo"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["group_memo"], str
                    )
                    tmp_res_data_this["member_count"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["member_count"], int
                    )
                    tmp_res_data_this["max_member_count"] = (
                        init_api_do_mapping_for_dict(
    ...
    def get_group_member_info(target_event, group_id, user_id, no_cache=False):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                ...
                res_data["data"]["user_id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["user_id"], int)
                )
                res_data["data"]["group_id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["group_id"], int)
                )
                res_data["data"]["times"]["join_time"] = init_api_do_mapping_for_dict(
                    raw_obj, ["join_time"], int
                )
                res_data["data"]["times"]["last_sent_time"] = (
                    init_api_do_mapping_for_dict(raw_obj, ["last_sent_time"], int)
                )
                res_data["data"]["times"]["shut_up_timestamp"] = (
                    init_api_do_mapping_for_dict(raw_obj, ["shut_up_timestamp"], int)
                )
                res_data["data"]["role"] = init_api_do_mapping_for_dict(
                    raw_obj, ["role"], str
                )
                res_data["data"]["card"] = init_api_do_mapping_for_dict(
                    raw_obj, ["card"], str
                )
                res_data["data"]["title"] = init_api_do_mapping_for_dict(
                    raw_obj, ["title"], str
                )
        return res_data
        ...
        if raw_obj is not None:
            if type(raw_obj) == list:
                ...
                for raw_obj_this in raw_obj:
                    ...
                    tmp_res_data_this["user_id"] = str(
                        init_api_do_mapping_for_dict(raw_obj_this, ["user_id"], int)
                    )
                    tmp_res_data_this["group_id"] = str(
                        init_api_do_mapping_for_dict(raw_obj_this, ["group_id"], int)
                    )
                    tmp_res_data_this["times"]["join_time"] = (
                        init_api_do_mapping_for_dict(raw_obj_this, ["join_time"], int)
                    )
                    tmp_res_data_this["times"]["last_sent_time"] = (
                        init_api_do_mapping_for_dict(
                            raw_obj_this, ["last_sent_time"], int
                        )
                    )
                    tmp_res_data_this["times"]["shut_up_timestamp"] = (
                        init_api_do_mapping_for_dict(
                            raw_obj_this, ["shut_up_timestamp"], int
                        )
                    )
                    tmp_res_data_this["role"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["role"], str
                    )
                    tmp_res_data_this["card"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["card"], str
                    )
                    tmp_res_data_this["title"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["title"], str
                    )
                    res_data["data"].append(tmp_res_data_this)
        return res_data
    ...
    def can_send_image(target_event):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                res_data["active"] = True
                res_data["data"]["yes"] = init_api_do_mapping_for_dict(
                    raw_obj, ["yes"], bool
                )
        return res_data
    ...
    def get_status(target_event):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                res_data["active"] = True
                res_data["data"]["online"] = init_api_do_mapping_for_dict(
                    raw_obj, ["online"], bool
                )
                res_data["data"]["status"]["packet_received"] = (
                    init_api_do_mapping_for_dict(
                        raw_obj, ["stat", "packet_received"], int
                    )
                )
                res_data["data"]["status"]["packet_sent"] = (
                    init_api_do_mapping_for_dict(raw_obj, ["stat", "packet_sent"], int)
                )
                res_data["data"]["status"]["packet_lost"] = (
                    init_api_do_mapping_for_dict(raw_obj, ["stat", "packet_lost"], int)
                )
                res_data["data"]["status"]["message_received"] = (
                    init_api_do_mapping_for_dict(
                        raw_obj, ["stat", "message_received"], int
                    )
                )
                res_data["data"]["status"]["message_sent"] = (
                    init_api_do_mapping_for_dict(raw_obj, ["stat", "message_sent"], int)
                )
                res_data["data"]["status"]["disconnect_times"] = (
                    init_api_do_mapping_for_dict(
                ...
                res_data["data"]["status"]["last_message_time"] = (
                    init_api_do_mapping_for_dict(
    ...
    def get_version_info(target_event):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                res_data["active"] = True
                res_data["data"]["name"] = init_api_do_mapping_for_dict(
                    raw_obj, ["app_name"], str
                )
                res_data["data"]["version_full"] = init_api_do_mapping_for_dict(
                    raw_obj, ["app_full_name"], str
                )
                res_data["data"]["version"] = init_api_do_mapping_for_dict(
                    raw_obj, ["app_version"], str
                )
                res_data["data"]["path"] = init_api_do_mapping_for_dict(
                    raw_obj, ["coolq_directory"], str
                )
                res_data["data"]["os"] = init_api_do_mapping_for_dict(
                    raw_obj, ["runtime_os"], str
                )
        return res_data
    ...
    def get_guild_member_profile(target_event, guild_id, user_id):
        res_data = OlivOS.contentAPI.api_result_data_template.get_group_member_info()
        raw_obj = None
        this_msg = api.get_guild_member_profile(
            get_SDK_bot_info_from_Event(target_event)
        )
        this_msg.data.guild_id = int(guild_id)
        this_msg.data.user_id = int(user_id)
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                ...
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["tiny_id"], str)
                )
                res_data["data"]["user_id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["tiny_id"], str)
                )
