...
class server(OlivOS.API.Proc_templet):
    def __init__(
        ...
        self.Proc_data["bot_info_dict_sdk"] = (
            OlivOS.onebotV12SDK.get_SDK_bot_info_from_Plugin_bot_info(bot_info_dict)
        )
    ...
    def run(self):
        self.log(
            ...
            OlivOS.L10NAPI.getTrans(
                "OlivOS onebotV12 link server [{0}] is running",
                [self.Proc_name],
                modelName,
            ),
        )
            ...
            if tmp_data_rx_obj.active:
                if tmp_data_rx_obj.data.type == "rxPacket":
                    sdk_event = OlivOS.onebotV12SDK.event(
                        tmp_data_rx_obj.data.data, self.Proc_data["bot_info_dict_sdk"]
                    )
                    if sdk_event.active:
    ...
    def message_router(self):
        while True:
            if (
                self.Proc_data["extend_data"]["ws_obj"] is None
                or self.Proc_info.rx_queue.empty()
            ):
