...
def get_Event_from_SDK(target_event: OlivOS.API.Event):
    ...
    if (
        ...
        waitForResSet(target_event.sdk_event.json["echo"], target_event.sdk_event.json)
    elif (
        ...
        if (
            ...
            target_event.data = target_event.private_message(
                str(target_event.sdk_event.json["user_id"]), "", "private"
            )
            target_event.data.message_sdk = OlivOS.messageAPI.Message_templet(
                "obv12_para", target_event.sdk_event.json["message"]
            )
            ...
            if "user_name" in target_event.sdk_event.json:
                ...
                target_event.data.sender["nickname"] = target_event.sdk_event.json[
                    "user_name"
                ]
            pass
        elif (
        ...
        elif "detail_type" in target_event.sdk_event.json and (
...
class api_templet(object):
    ...
    def do_api(self, bot_hash, control_queue):
        data = self.do_dump()
        send_ws_event(hash=bot_hash, data=data, control_queue=control_queue)

    def do_dump(self):
        res_obj = {"action": self.action, "params": {}, "echo": self.echo}
        if self.params != None:
...
def send_ws_event(hash, data, control_queue):
    sendControlEventSend(
        "send",
        {
            "target": {"type": "onebotV12_link", "hash": hash},
            ...
            if type(raw_obj) == list:
                ...
                for raw_obj_this in raw_obj:
                    ...
                    tmp_res_data_this["name"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["user_name"], str
                    )
                    tmp_res_data_this["id"] = init_api_do_mapping_for_dict(
                        raw_obj_this, ["user_id"], str
                    )
    ...
    def get_group_member_info(target_event: OlivOS.API.Event, group_id, user_id):
        ...
        if raw_obj is not None:
            if type(raw_obj) is dict:
                ...
                res_data["data"]["user_id"] = init_api_do_mapping_for_dict(
                    raw_obj, ["user_id"], str
                )
        ...
        if raw_obj is not None:
            if type(raw_obj) == list:
                ...
                for raw_obj_this in raw_obj:
                    ...
                    tmp_res_data_this["data"]["name"] = init_api_do_mapping_for_dict(
                        raw_obj, ["user_name"], str
                    )
                    tmp_res_data_this["data"]["id"] = init_api_do_mapping_for_dict(
                        raw_obj, ["user_id"], str
                    )
                    tmp_res_data_this["data"]["user_id"] = init_api_do_mapping_for_dict(
                        raw_obj, ["user_id"], str
                    )
                    ...
                    tmp_res_data_this["data"]["card"] = init_api_do_mapping_for_dict(
                        raw_obj, ["user_remark"], str
                    )
...
def init_api_json(raw: dict):
    ...
    if (
        ...
        flag_is_active = True
    if flag_is_active and "data" in tmp_obj:
