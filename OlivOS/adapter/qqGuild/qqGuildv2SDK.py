...
class PAYLOAD(object):
    ...
    class sendIdentify(payload_template):
        def __init__(
            self,
            bot_info: bot_info_T,
            intents=(int(intents_T.GUILDS) | int(intents_T.DIRECT_MESSAGE)),
        ):
            tmp_intents = intents
...
def get_Event_from_SDK(target_event):
    ...
    if plugin_event_bot_hash not in sdkSubSelfInfo:
        ...
        if (
            ...
            sdkSubSelfInfo[plugin_event_bot_hash] = str(
                sdkSelfInfo[plugin_event_bot_hash]["id"]
            )
    if target_event.sdk_event.payload.data.t == "READY":
        ...
        if (
            ...
            sdkSelfInfo[plugin_event_bot_hash] = target_event.sdk_event.payload.data.d[
                "user"
            ]
        ...
        if "attachments" in target_event.sdk_event.payload.data.d:
            if type(target_event.sdk_event.payload.data.d["attachments"]) == list:
                for attachments_this in target_event.sdk_event.payload.data.d[
                    "attachments"
                ]:
        ...
        if message_obj.active:
            ...
            target_event.data.extend["group_id"] = str(
                target_event.sdk_event.payload.data.d["group_id"]
            )
            ...
            target_event.data.extend["reply_msg_id"] = (
                target_event.sdk_event.payload.data.d["id"]
            )
            if "member" in target_event.sdk_event.payload.data.d:
                if "roles" in target_event.sdk_event.payload.data.d["member"]:
                    tmp_role_now = target_event.sdk_event.payload.data.d["member"][
                        "roles"
                    ]
        ...
        if message_obj.active:
            ...
            target_event.data.sender["nickname"] = (
                target_event.sdk_event.payload.data.d["author"]["username"]
            )
            target_event.data.sender["id"] = str(
                target_event.sdk_event.payload.data.d["author"]["id"]
            )
            target_event.data.sender["name"] = target_event.sdk_event.payload.data.d[
                "author"
            ]["username"]
            target_event.data.sender["sex"] = "unknown"
            target_event.data.sender["age"] = 0
            target_event.data.sender["role"] = "member"
            target_event.data.host_id = target_event.sdk_event.payload.data.d[
                "guild_id"
            ]
            target_event.data.extend["group_id"] = str(
            ...
            target_event.data.extend["host_group_id"] = str(
                target_event.sdk_event.payload.data.d["guild_id"]
            )
            target_event.data.extend["flag_from_direct"] = False
            target_event.data.extend["reply_msg_id"] = (
                target_event.sdk_event.payload.data.d["id"]
...
class event_action(object):
    def send_qq_msg(
        target_event, chat_id, message, reply_msg_id=None, flag_direct=False
    ):
        this_msg = None
        msg_id = None
        if flag_direct:
            this_msg = API.sendQQDirectMessage(
    ...
    def get_login_info(target_event):
        ...
        if (
            ...
            res_data["data"]["name"] = str(
                sdkSelfInfo[target_event.bot_info.hash]["username"]
            )
            ...
            try:
                ...
                if raw_obj is not None:
                    if type(raw_obj) is dict and 0 == init_api_do_mapping_for_dict(
                        raw_obj, ["code"], int
                    ):
                        res_data["active"] = True
                        res_data["data"]["name"] = init_api_do_mapping_for_dict(
                            raw_obj, ["username"], str
                        )
                        res_data["data"]["id"] = str(
                            init_api_do_mapping_for_dict(raw_obj, ["id"], str)
                        )
            except:
    ...
    def setResourceUploadFast(
        target_event, url: str, type_path: str = "images", type_chat: str = "qq_groups"
    ):
        res = None
        check_list = {
