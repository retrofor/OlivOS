...
def get_message_obj(elements: list):
    ...
    for element_this in elements:
        if (
            ...
            message_list.append(
                OlivOS.messageAPI.PARA.at(id=element_this["textElement"]["atNtUin"])
            )
            flag_hit = True
        elif (
            "textElement" in element_this
            ...
            message_list.append(
                OlivOS.messageAPI.PARA.text(text=element_this["textElement"]["content"])
            )
            flag_hit = True
    if not flag_hit:
...
def get_Event_from_SDK(target_event):
    ...
    if target_event.sdk_event.payload.active:
        if (
            target_event.sdk_event.payload.type == "message::recv"
            ...
            if (
                ...
                if message_obj.active:
                    ...
                    if len(payload_data["sendMemberName"]) > 0:
                        target_event.data.sender["nickname"] = payload_data[
                            "sendMemberName"
                        ]
                    target_event.data.sender["id"] = target_event.data.sender["user_id"]
                    target_event.data.sender["name"] = target_event.data.sender[
                        "nickname"
                    ]
                ...
                "chatType" in payload_data
                and payload_data["chatType"] == 1
                and "peerUin" in payload_data
                and type(payload_data["peerUin"]) is str
                and "elements" in payload_data
                and type(payload_data["elements"]) is list
                and len(payload_data["elements"]) > 0
            ):
...
class PAYLOAD(object):
    ...
    class messageSend(payload_template):
        def __init__(
            self, target_type, target_id, message: OlivOS.messageAPI.Message_templet
        ):
            payload_template.__init__(self)
...
def send_ws_event(hash, data, control_queue):
    sendControlEventSend(
        "send",
        {
            "target": {"type": "qqRed_link", "hash": hash},
