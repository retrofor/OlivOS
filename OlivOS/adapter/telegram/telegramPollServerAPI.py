    ...
    def run_poll_list(self):
        for bot_info_this in self.Proc_data["bot_info_dict"]:
            ...
            if bot_info_this_obj.platform["sdk"] == "telegram_poll":
                ...
                sdk_bot_info_this = (
                    OlivOS.telegramSDK.get_SDK_bot_info_from_Plugin_bot_info(
                        bot_info_this_obj
                    )
                )
                sdk_api_tmp = OlivOS.telegramSDK.API.getUpdates(sdk_bot_info_this)
                sdk_api_tmp.data.offset = self.Proc_data["bot_info_update_id"][
                    bot_info_this
                ]
                try:
                    sdk_api_tmp.do_api()
                    ...
                    try:
                        ...
                        if type(res_obj["result"]) == list and res_obj["result"] != []:
                            self.Proc_data["bot_info_update_id"][bot_info_this] = (
                                res_obj["result"][-1]["update_id"] + 1
                            )
                            if not flag_first_update:
                                for result_this in res_obj["result"]:
                                    sdk_event = OlivOS.telegramSDK.event(
                                        result_this, "poll", bot_info_this_obj
                                    )
