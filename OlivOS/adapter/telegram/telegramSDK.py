            ...
            if checkByListAnd([
                ...
                for entities_this in target_event.sdk_event.json["message"]["entities"]:
                    if entities_this["type"] == "mention":
                        ...
                        tmp_message_raw_2 = ""
                        tmp_message_raw_3 = ""
                        tmp_message_raw_1 = tmp_message_raw[
                            tmp_message_offset_count : entities_this["offset"]
                        ]
                        tmp_message_raw_2 = tmp_message_raw[
                            entities_this["offset"] : entities_this["offset"]
                            + entities_this["length"]
                        ]
                        tmp_message_raw_2 = tmp_message_raw_2[1:]
                        tmp_message_raw_3 = tmp_message_raw[
                            entities_this["offset"] + entities_this["length"] :
                        ]
                        tmp_message_offset_count = (
                            entities_this["offset"] + entities_this["length"]
                        )
                        if len(tmp_message_raw_1) > 0:
                            message_list.append(
                                OlivOS.messageAPI.PARA.text(text=tmp_message_raw_1)
                            )
                        if len(tmp_message_raw_2) > 0:
                            message_list.append(
                                OlivOS.messageAPI.PARA.at(id=tmp_message_raw_2)
                            )
...
def get_Event_from_SDK(target_event):
    ...
    if plugin_event_bot_hash not in sdkSubSelfInfo:
        ...
        try:
            ...
            if tmp_api_json["ok"] == True:
                sdkSubSelfInfo[plugin_event_bot_hash] = tmp_api_json["result"][
                    "username"
                ]
        except:
            pass
    if checkByListAnd([
        ...
        target_event.data.message_id = str(
            target_event.sdk_event.json["message"]["message_id"]
        )
        ...
        target_event.data.sender["nickname"] = target_event.sdk_event.json["message"][
            "from"
        ]["first_name"]
        target_event.data.sender["id"] = str(
            target_event.sdk_event.json["message"]["from"]["id"]
        )
        target_event.data.sender["name"] = target_event.sdk_event.json["message"][
            "from"
        ]["first_name"]
    ...
    if checkByListAnd([
        ...
        target_event.data.message_sdk = message_obj
        target_event.data.message_id = target_event.sdk_event.json["message"][
            "message_id"
        ]
        target_event.data.raw_message = message_obj
        target_event.data.raw_message_sdk = message_obj
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                ...
                res_data["data"]["name"] = init_api_do_mapping_for_dict(
                    raw_obj, ["first_name"], str
                )
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["id"], int)
                )
        return res_data
    ...
    def get_group_member_info(target_event, chat_id, user_id):
        ...
        if raw_obj is not None:
            if type(raw_obj) == dict:
                res_data["active"] = True
                res_data["data"]["name"] = init_api_do_mapping_for_dict(
                    raw_obj, ["user", "first_name"], str
                )
                res_data["data"]["id"] = str(
                    init_api_do_mapping_for_dict(raw_obj, ["user", "id"], int)
                )
                ...
                tmp_role = init_api_do_mapping_for_dict(
                    raw_obj, ["status", "role"], str
                )
                if tmp_role == "creator":
