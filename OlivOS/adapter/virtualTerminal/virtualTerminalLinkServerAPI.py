...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        time.sleep(2)
        self.log(
            2, "OlivOS virtual terminal link server [" + self.Proc_name + "] is running"
        )
                ...
                if self.Proc_info.rx_queue.empty():
                    ...
                    if (
                        ...
                        if "reply" == rx_packet_data.key["data"]["action"]:
                            if (
                                "data" in rx_packet_data.key["data"]
                                and "event_id" in rx_packet_data.key["data"]
                            ):
                                self.Proc_data["reply_event_pool"][
                                    str(rx_packet_data.key["data"]["event_id"])
                ...
                if flag_active:
                    for count_i in range(30 * 4):
                        if event_id in self.Proc_data["reply_event_pool"]:
                            res = json.dumps(
                                self.Proc_data["reply_event_pool"][event_id]
                            )
