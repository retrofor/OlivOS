...
def get_Event_from_SDK(target_event):
    ...
    if target_event.platform["model"] in ["default"]:
        message_obj = OlivOS.messageAPI.Message_templet(
            "olivos_string", target_event.sdk_event.payload.key["data"]["data"]
        )
    ...
    if message_obj is not None and target_event.platform["model"] in ["default"]:
        if message_obj.active:
            ...
            if tmp_user_conf["flag_group"] == True:
                ...
                target_event.data = target_event.private_message(
                    tmp_user_conf[
                        "user_id"
                    ],  # 此时的 user_id 为发送者的 user_id，接收者固定为机器人
                    message_obj,
                    "private",
                )
            ...
            if tmp_user_conf["flag_group"] and "unknown" != tmp_user_conf["group_role"]:
        ...
        # 此段内容不修改
        if (
            "type" in target_event.sdk_event.payload
            and target_event.sdk_event.payload["type"] == "message"
        ):
            if (
                ...
                message_obj = OlivOS.messageAPI.Message_templet(
                    "olivos_string", target_event.sdk_event.payload["message"]
                )
                if message_obj.active:
                    try:
                        target_event.active = True
                        ...
                        target_event.data.sender["id"] = str(
                            target_event.sdk_event.payload["user_id"]
                        )
                        if (
                            ...
                            target_event.data.sender["name"] = (
                                target_event.sdk_event.payload["sender"]["nickname"]
                            )
                        target_event.data.sender["nickname"] = target_event.data.sender[
                            "name"
                        ]
                        target_event.data.sender["user_id"] = target_event.data.sender[
                            "id"
                        ]
                        target_event.data.sender["sex"] = "unknown"
                        target_event.data.sender["age"] = 0
                        ...
                        if target_event.platform["model"] in ["ff14"]:
                            ...
                            hash_tmp.update(
                                str(target_event.data.sender["name"]).encode(
                                    encoding="UTF-8"
                                )
                            )
                            tmp_hash_user_id = str(
                                int(int(hash_tmp.hexdigest(), 16) % 1000000000) + 1
                            )
...
class event_action:
    @staticmethod
    def send_msg(
        target_event, message, control_queue, flag_type="group", target_id=None
    ):
        plugin_event_bot_hash = OlivOS.API.getBotHash(
            bot_id=target_event.base_info["self_id"],
            platform_sdk=target_event.platform["sdk"],
        ...
        if target_event.platform["model"] in ["default"]:
            if target_event.active:
                ...
                send_log_event(
                    plugin_event_bot_hash, message, "BOT", control_queue, user_conf
                )
            ...
            if target_event.sdk_event.event_id is not None:
                ...
                event_data = {"reply": message}
                send_postapi_event(
                    plugin_event_bot_hash, event_data, evnet_id, control_queue
                )
...
def send_postapi_event(hash, data, event_id, control_queue):
    sendControlEventSend(
        "send",
        {
            ...
            "data": {"action": "reply", "event_id": str(event_id), "data": data},
        },
        control_queue,
    )
...
def send_ff14_post(plugin_event, message: str):
    if True:
        send_url = "http://127.0.0.1:%s/Command" % str(
            plugin_event.bot_info.post_info.access_token
        )
