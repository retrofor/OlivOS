...
"""
        ...
        for basic_conf_models_this_name in main_control.init_list:
            main_control.control_queue.put(
                main_control.packet("init", basic_conf_models_this_name), block=False
            )
        ...
        while True:
            ...
            if rx_packet_data.action == "init":
                ...
                if basic_conf_models_this["enable"]:
                    ...
                    if basic_conf_models_this["type"] == "sleep":
                        ...
                        threading.Thread(
                            ...
                            kwargs={
                                "logger_proc": Proc_dict[
                                    basic_conf_models_this["logger_proc"]
                                ],
                                "flagChackOnly": True,
                        ...
                        tmp_tx_queue_list = []
                        for tmp_tx_queue_list_this in basic_conf_models_this[
                            "tx_queue"
                        ]:
                            tmp_tx_queue_list.append(
                                multiprocessing_dict[tmp_tx_queue_list_this]
                            )
                        Proc_dict[basic_conf_models_this["name"]] = (
                            OlivOS.pluginAPI.shallow(
                                ...
                                enable_auto_restart=basic_conf_models_this[
                                    "enable_auto_restart"
                                ],
                            )
                        )
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                tmp_Proc_name = (
                                    basic_conf_models_this["name"] + "=" + bot_info_key
                                )
                                tmp_queue_name = (
                                    basic_conf_models_this["rx_queue"]
                                    + "="
                                ...
                                multiprocessing_dict[tmp_queue_name] = (
                                    multiprocessing.Queue()
                                )
                                tmp_Proc_name = (
                                    basic_conf_models_this["name"] + "=" + bot_info_key
                                )
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.virtualTerminalLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                                        dead_interval=basic_conf_models_this[
                                            "dead_interval"
                                        ],
                                        rx_queue=multiprocessing_dict[tmp_queue_name],
                                        tx_queue=multiprocessing_dict[
                                            basic_conf_models_this["tx_queue"]
                                        ],
                                        control_queue=multiprocessing_dict[
                                            basic_conf_models_this["control_queue"]
                                        ],
                                ...
                                Proc_Proc_dict[tmp_Proc_name] = Proc_dict[
                                    tmp_Proc_name
                                ].start_unity(tmp_proc_mode)
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    plugin_bot_info_dict[bot_info_key].platform["model"]
                                    in OlivOS.flaskServerAPI.gCheckList
                                ):
                                    flag_need_enable = True
                        if not flag_need_enable:
                            continue
                        Proc_dict[basic_conf_models_this["name"]] = (
                            OlivOS.flaskServerAPI.server(
                                Proc_name=basic_conf_models_this["name"],
                                scan_interval=basic_conf_models_this["interval"],
                                dead_interval=basic_conf_models_this["dead_interval"],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    plugin_bot_info_dict[bot_info_key].platform["model"]
                                    in OlivOS.onebotV12LinkServerAPI.gCheckList
                                ):
                                    flag_need_enable = True
                        if not flag_need_enable:
                            continue
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    ...
                                    Proc_dict[tmp_Proc_name] = (
                                        OlivOS.onebotV12LinkServerAPI.server(
                                            Proc_name=tmp_Proc_name,
                                            scan_interval=basic_conf_models_this[
                                                "interval"
                                            ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    plugin_bot_info_dict[bot_info_key].platform["model"]
                                    in OlivOS.qqRedLinkServerAPI.gCheckList
                                ):
                                    flag_need_enable = True
                        if not flag_need_enable:
                            continue
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    ...
                                    Proc_dict[tmp_Proc_name] = (
                                        OlivOS.qqRedLinkServerAPI.server(
                                            Proc_name=tmp_Proc_name,
                                            scan_interval=basic_conf_models_this[
                                                "interval"
                                            ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    plugin_bot_info_dict[bot_info_key].platform["model"]
                                    in OlivOS.dingtalkLinkServerAPI.gCheckList
                                ):
                                    flag_need_enable = True
                        if not flag_need_enable:
                            continue
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    ...
                                    Proc_dict[tmp_Proc_name] = (
                                        OlivOS.dingtalkLinkServerAPI.server(
                                            Proc_name=tmp_Proc_name,
                                            scan_interval=basic_conf_models_this[
                                                "interval"
                                            ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.qqGuildLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.qqGuildv2LinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.discordLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.kaiheilaLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.mhyVilaLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.biliLiveLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.hackChatLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    plugin_bot_info_dict[bot_info_key].platform["model"]
                                    in OlivOS.OPQBotLinkServerAPI.gCheckList
                                ):
                                    flag_need_enable = True
                        if not flag_need_enable:
                            continue
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                if (
                                    ...
                                    Proc_dict[tmp_Proc_name] = (
                                        OlivOS.OPQBotLinkServerAPI.server(
                                            Proc_name=tmp_Proc_name,
                                            scan_interval=basic_conf_models_this[
                                                "interval"
                                            ],
                        ...
                        if not flag_need_enable:
                            continue
                        Proc_dict[basic_conf_models_this["name"]] = (
                            OlivOS.telegramPollServerAPI.server(
                                Proc_name=basic_conf_models_this["name"],
                                scan_interval=basic_conf_models_this["interval"],
                                dead_interval=basic_conf_models_this["dead_interval"],
                                rx_queue=None,
                        ...
                        if not flag_need_enable:
                            continue
                        Proc_dict[basic_conf_models_this["name"]] = (
                            OlivOS.fanbookPollServerAPI.server(
                                Proc_name=basic_conf_models_this["name"],
                                scan_interval=basic_conf_models_this["interval"],
                                dead_interval=basic_conf_models_this["dead_interval"],
                                rx_queue=None,
                        ...
                        for bot_info_key in plugin_bot_info_dict:
                            if (
                                ...
                                Proc_dict[tmp_Proc_name] = (
                                    OlivOS.dodoLinkServerAPI.server(
                                        Proc_name=tmp_Proc_name,
                                        scan_interval=basic_conf_models_this[
                                            "interval"
                                        ],
                        ...
                        if not flag_need_enable:
                            continue
                        Proc_dict[basic_conf_models_this["name"]] = (
                            OlivOS.dodoPollServerAPI.server(
                                Proc_name=basic_conf_models_this["name"],
                                scan_interval=basic_conf_models_this["interval"],
                                dead_interval=basic_conf_models_this["dead_interval"],
                                rx_queue=None,
                        ...
                        Proc_dict[basic_conf_models_this["name"]] = (
                            OlivOS.dodobotEAServerAPI.server(
                                Proc_name=basic_conf_models_this["name"],
                                scan_interval=basic_conf_models_this["interval"],
                                dead_interval=basic_conf_models_this["dead_interval"],
                                rx_queue=None,
                        ...
                        basic_conf_models_this["type"] == "multiLoginUI"
                        ...
                        if platform.system() == "Windows":
                            ...
                            if basic_conf_models_this["name"] not in Proc_Proc_dict:
                                Proc_Proc_dict[basic_conf_models_this["name"]] = (
                                    Proc_dict[
                                        basic_conf_models_this["name"]
                                    ].start_unity(tmp_proc_mode)
                                )
                        ...
                        if platform.system() == "Windows":
                            if not OlivOS.libAstralQsignEXEModelAPI.isBotActive(
                                plugin_bot_info_dict
                            ):
                                continue
                            threading.Thread(
                ...
                main_control.control_queue.put(
                    OlivOS.API.Control.packet("init_type", Proc_type_this), block=False
                )
                ...
                for tmp_Proc_name in basic_conf_models:
                    ...
                    if basic_conf_models_this["type"] == rx_packet_data.key:
                        if (
                            Proc_dict[basic_conf_models_this["name"]].Proc_info.rx_queue
                            is not None
                        ):
                            Proc_dict[
                                basic_conf_models_this["name"]
                            ].Proc_info.rx_queue.put(
                                OlivOS.API.Control.packet(
                                    "restart_do", basic_conf_models_this["name"]
                                ),
                                block=False,
                            )
                ...
                if type(rx_packet_data.key) == dict:
                    if "target" in rx_packet_data.key:
                        if "type" in rx_packet_data.key["target"]:
                            flag_target_all = (
                                rx_packet_data.key["target"]["type"] == "all"
                            )
                            flag_fliter = "all"
                            if "fliter" in rx_packet_data.key["target"]:
                                flag_fliter = rx_packet_data.key["target"]["fliter"]
                            for tmp_Proc_name in basic_conf_models:
                                basic_conf_models_this = basic_conf_models[
                                    tmp_Proc_name
                                ]
                                if (
                                    flag_target_all
                                    or basic_conf_models_this["type"]
                                    == rx_packet_data.key["target"]["type"]
                                ):
                                    model_name = basic_conf_models_this["name"]
                                        ...
                                        if flag_fliter in ["all", "control_only"]:
                                            try:
                                                if (
                                                    Proc_dict[
                                                        model_name
                                                    ].Proc_info.control_rx_queue
                                                    is not None
                                                ):
                                                    Proc_dict[
                                                        model_name
                                                    ].Proc_info.control_rx_queue.put(
                                                        rx_packet_data, block=False
                                                    )
                            ...
                            for basic_conf_models_this_key in basic_conf_models:
                                ...
                                if "webview_page" == basic_conf_models_this["type"]:
                                    if basic_conf_models_this["name"] not in Proc_dict:
                                        ...
                                        Proc_dict[model_name] = (
                                            OlivOS.webviewUIAPI.page(
                                    ...
                                    if model_name not in Proc_Proc_dict:
                                        Proc_Proc_dict[model_name] = Proc_dict[
                                            model_name
                                        ].start_unity("processing")
                    ...
                    if (
                        ...
                        if "init" == rx_packet_data.key["target"]["action"]:
                            for basic_conf_models_this_key in basic_conf_models:
                                ...
                                if (
                                    "tx_turingTest_webview_page"
                                    == basic_conf_models_this["type"]
                                ):
                                    if basic_conf_models_this["name"] not in Proc_dict:
                                        model_name = "%s+%s" % (
                                            basic_conf_models_this["name"],
                                        ...
                                        Proc_dict[model_name] = (
                                            OlivOS.libEXEModelAPI.txTuringTestPage(
                                                Proc_name=model_name,
                                                scan_interval=basic_conf_models_this[
                                                    "interval"
                                                ],
                    ...
                    for event_this in rx_packet_data.key["action"]:
                        if (
                            ...
                            for model_this in basic_conf["system"]["event"][event_this]:
                                main_control.control_queue.put(
                                    main_control.packet("init", model_this), block=False
                                )
                ...
                if (
                    ...
                    for event_this in rx_packet_data.key["action"]:
                        if (
                            ...
                            for model_this in basic_conf["system"]["type_event"][
                                event_this
                            ]:
                                main_control.control_queue.put(
                ...
                if (
                    ...
                    main_control.control_queue.put(
                        main_control.packet(
                            "send",
                            {
                                "target": {"type": "all", "fliter": "control_only"},
                                "data": {
...
def update_get_func(Proc_dict, basic_conf_models, basic_conf_models_this):
            ...
            if basic_conf_models_this["type"] == "plugin":
                if (
                    ...
                    Proc_dict[basic_conf_models_this["name"]].Proc_info.rx_queue.put(
                        OlivOS.API.Control.packet(
                            "update_hit", basic_conf_models_this["name"]
                        ),
                        block=False,
                    )
...
def bootMonitor(varDict: dict):
    ...
    try:
        ...
        if "Proc_dict" in varDict and type(varDict["Proc_dict"]) is dict:
            ...
            for Proc_name in gMonitorReg["Proc_dict"]["keys"]:
                if Proc_name not in varDict["Proc_dict"]:
                    ...
                    logG(
                        2,
                        OlivOS.L10NAPI.getTrans(
                            "OlivOS model [{0}] stopped", [Proc_name], modelName
                        ),
                    )
            for Proc_name in varDict["Proc_dict"]:
                if Proc_name not in gMonitorReg["Proc_dict"]["keys"]:
                    ...
                    logG(
                        2,
                        OlivOS.L10NAPI.getTrans(
                            "OlivOS model [{0}] init", [Proc_name], modelName
                        ),
                    )
            if flagNeedRefresh:
                gMonitorReg["Proc_dict"]["keys"] = copy.deepcopy(
                    list(varDict["Proc_dict"].keys())
                )
    except Exception as e:
        traceback.print_exc()
