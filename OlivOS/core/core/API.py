...
def getBotHash(
    bot_id=None, platform_sdk=None, platform_platform=None, platform_model=None
):
...
class Event(object):
    def __init__(self, sdk_event=None, log_func=None, Proc=None):
        ...
        if type(OlivOS.pluginAPI.gProc) is OlivOS.pluginAPI.shallow:
            self.plugin_info["control_queue"] = (
                OlivOS.pluginAPI.gProc.Proc_info.control_queue
            )
    ...
    def __init_inde_interface(self):
        ...
        if self.platform["sdk"] == "kaiheila_link":
            self.indeAPI = OlivOS.kaiheilaSDK.inde_interface(
                self, self.platform["platform"]
            )
        if self.platform["sdk"] == "mhyVila_link":
            self.indeAPI = OlivOS.mhyVilaSDK.inde_interface(
                self, self.platform["platform"]
            )
        ...
        if self.active:
            ...
            if self.plugin_info["func_type"] in OlivOS.metadataAPI.eventLogMetaTable:
                ...
                tmp_log_message = OlivOS.metadataAPI.getTextByMetaTableFormat(
                    ...
                    fwd_key=OlivOS.metadataAPI.eventLogMetaTable[
                        self.plugin_info["func_type"]
                    ]["message_key"],
    ...
    class group_member_decrease(object):
        def __init__(
            self, group_id, operator_id, user_id, action="leave", flag_lazy=True
        ):
            self.group_id = group_id
    ...
    class group_member_increase(object):
        def __init__(
            self, group_id, operator_id, user_id, action="approve", flag_lazy=True
        ):
            self.group_id = group_id
            self.operator_id = operator_id
            self.user_id = user_id
    ...
    def set_block(
        self, enable: bool = True, flag_log: bool = True, remote: bool = False
    ):
        if remote:
            pass
        else:
    ...
    def __message_router(self, message):
        ...
        if type(message) == str or type(message) == list:
            tmp_message_obj = OlivOS.messageAPI.Message_templet(
                self.plugin_info["message_mode_tx"], message
            )
        elif type(message) == OlivOS.messageAPI.Message_templet:
            tmp_message_obj = message
        else:
            error_note = (
                "Wrong message type from plugin, please check your plugin first"
            )
            error_obj = OlivOS.contentAPI.api_result_error_template.OlivOSTypeError(
                error_note
            )
    ...
    def __reply(self, message, flag_log=True):
        ...
        if checkByListOrEqual(
            ...
            if hasattr(self.data, "extend") and "host_group_id" in self.data.extend:
                self.__send(
                    "private",
                    self.data.user_id,
                    tmp_message,
            ...
            self.plugin_info["func_type"], ["group_message_sent", "group_message"]
        ):
            self.__send(
                "group",
                self.data.group_id,
            ...
            flag_type = "group"
        elif checkByListOrEqual(self.plugin_info["func_type"], ["poke"]):
        ...
        if flag_log and self.log_func is not None:
            if tmp_message_obj.active:
                tmp_message_log = tmp_message_obj.get(
            ...
            if flag_type == "private":
                self.log_func(
                    2,
                    "User(" + str(self.data.user_id) + "): " + tmp_message_log,
                    [
                        (self.getBotIDStr(), "default"),
                        (self.plugin_info["name"], "default"),
                        ("reply", "callback"),
                ...
                if checkByListOrEqual(
                    self.plugin_info["func_type"],
                    ["group_message_sent", "group_message"],
                ):
                    if self.data.host_id is not None:
                        self.log_func(
                            2,
                            "Host("
                            + str(self.data.host_id)
                            + ") Group("
                            + str(self.data.group_id)
                            + "): "
                            + tmp_message_log,
                            [
                                (self.getBotIDStr(), "default"),
                    ...
                    self.log_func(
                        2,
                        "Group(" + str(self.data.group_id) + "): " + tmp_message_log,
                        [
                            (self.getBotIDStr(), "default"),
                            (self.plugin_info["name"], "default"),
                            ("reply", "callback"),
    ...
    def __send(self, send_type, target_id, message, host_id=None, flag_log=True):
        ...
        if self.platform["sdk"] == "terminal_link":
            ...
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if flag_type == "private":
                    if (
                        ...
                        OlivOS.onebotV12SDK.event_action.send_private_msg(
                            self, target_id, tmp_message
                        )
                elif flag_type == "group":
                    if host_id is not None:
                        OlivOS.onebotV12SDK.event_action.send_host_msg(
                            self, host_id, target_id, tmp_message
                        )
                    else:
                        OlivOS.onebotV12SDK.event_action.send_group_msg(
                            self, target_id, tmp_message
                        )
                ...
                if flag_type == "private":
                    if (
                        ...
                        OlivOS.onebotSDK.event_action.send_private_msg(
                            self, target_id, tmp_message
                        )
                elif flag_type == "group":
                    if host_id is not None:
                        OlivOS.onebotSDK.event_action.send_guild_channel_msg(
                            self, host_id, target_id, tmp_message
                        )
                    else:
                        OlivOS.onebotSDK.event_action.send_group_msg(
                            self, target_id, tmp_message
                        )
            ...
            if flag_type == "group":
                if hasattr(self.data, "extend") and "reply_msg_id" in self.data.extend:
            ...
            if hasattr(self.data, "extend") and self.data.extend.get(
                "flag_from_qq", False
            ):
                if flag_type == "group":
                    if (
                ...
                if flag_type == "group":
                    ...
                    if hasattr(self.data, "extend") and host_id is not None:
                        OlivOS.qqGuildv2SDK.event_action.send_msg(
                            self,
                            host_id,
                            tmp_message,
            ...
            if flag_type == "group":
                ...
                OlivOS.discordSDK.event_action.send_msg(
                    self, target_id, tmp_message, flag_direct=True
                )
        elif self.platform["sdk"] == "kaiheila_link":
            if flag_type == "group":
                OlivOS.kaiheilaSDK.event_action.send_msg(
                    self, target_id, tmp_message, flag_direct=False
                )
            elif flag_type == "private":
                OlivOS.kaiheilaSDK.event_action.send_msg(
                    self, target_id, tmp_message, flag_direct=True
            ...
            if flag_type == "group":
                if host_id is not None:
                    OlivOS.mhyVilaSDK.event_action.send_group_msg(
                        self, target_id, tmp_message, host_id=host_id
                    )
            ...
            OlivOS.hackChatSDK.event_action.send_msg(
                self, tmp_message, self.plugin_info["control_queue"]
            )
        elif self.platform["sdk"] == "biliLive_link":
            OlivOS.biliLiveSDK.event_action.send_msg(
                self, tmp_message, self.plugin_info["control_queue"]
            )
            ...
            if flag_type == "private":
                OlivOS.fanbookSDK.event_action.send_private_msg(
                    self, target_id, tmp_message
                )
            elif flag_type == "group":
            ...
            if flag_type == "group":
                ...
                OlivOS.dodoLinkSDK.event_action.send_personal_msg(
                    self, target_id, tmp_message
                )
        elif self.platform["sdk"] == "dodo_poll":
            if flag_type == "private":
                if host_id is not None:
                    OlivOS.dodoSDK.event_action.send_private_msg(
                        self, host_id, target_id, tmp_message
                    )
                elif "host_id" in self.data.__dict__:
                    OlivOS.dodoSDK.event_action.send_private_msg(
                        self, self.data.host_id, target_id, tmp_message
                    )
            elif flag_type == "group":
                OlivOS.dodoSDK.event_action.send_msg(self, target_id, tmp_message)
        elif self.platform["sdk"] == "dodobot_ea":
            if flag_type == "group":
                tmp_send_msg = OlivOS.dodobotEASDK.event_action.send_msg(
                    self, target_id, tmp_message
                )
                tx_packet_data = OlivOS.dodobotEATXAPI.server.rx_packet(
                    "send", tmp_send_msg
                )
            ...
            OlivOS.dingtalkSDK.event_action.send_msg(
                self, flag_type, target_id, tmp_message
            )
        ...
        if flag_log and self.log_func is not None:
            ...
            if flag_type == "private":
                self.log_func(
                    2,
                    "User(" + str(target_id) + "): " + tmp_message_log,
                    [
                        (self.getBotIDStr(), "default"),
                        (self.plugin_info["name"], "default"),
                        ("send", "callback"),
                ...
                if host_id is not None:
                    ...
                    self.log_func(
                        2,
                        "Group(" + str(target_id) + "): " + tmp_message_log,
                        [
                            (self.getBotIDStr(), "default"),
                            (self.plugin_info["name"], "default"),
                            ("send", "callback"),
    ...
    def delete_msg(
        self, message_id: "str|int", flag_log: bool = True, remote: bool = False
    ):
        if remote:
            pass
        else:
    ...
    def __set_group_kick(
        self, group_id, user_id, host_id, rehect_add_request, flag_log=True
    ):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotV12SDK.event_action.set_group_kick(
                        self, group_id, user_id
                    )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_kick(
                        self, group_id, user_id, rehect_add_request
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def set_group_kick(
        ...
        if remote:
            ...
            self.__set_group_kick(
                group_id, user_id, host_id, rehect_add_request, flag_log=True
            )
    ...
    def __set_group_ban(self, group_id, user_id, host_id, duration, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotV12SDK.event_action.set_group_ban(
                        self, group_id, user_id, duration
                    )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_ban(
                        self, group_id, user_id, duration
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def __set_group_anonymous_ban(
        self, group_id, anonymous, anonymous_flag, host_id, duration, flag_log=True
    ):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                pass
    ...
    def set_group_anonymous_ban(
        ...
        if remote:
            ...
            self.__set_group_anonymous_ban(
                group_id, anonymous, anonymous_flag, host_id, duration, flag_log=True
            )
    ...
    def __set_group_whole_ban(self, group_id, enable, host_id, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                ...
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_whole_ban(
                        self, group_id, enable
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def __set_group_admin(self, group_id, user_id, enable, host_id, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotV12SDK.event_action.set_group_admin(
                        self, group_id, user_id, enable
                    )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_admin(
                        self, group_id, user_id, enable
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def __set_group_anonymous(self, group_id, enable, host_id, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                ...
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_anonymous(
                        self, group_id, enable
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def __set_group_card(self, group_id, user_id, card, host_id, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                ...
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_card(
                        self, group_id, user_id, card
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def __set_group_name(self, group_id, group_name, host_id, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotV12SDK.event_action.set_group_name(
                        self, group_id, group_name
                    )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_name(
                        self, group_id, group_name
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
    ...
    def __set_group_leave(self, group_id, host_id, is_dismiss, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                ...
                if host_id is None:
                    OlivOS.onebotSDK.event_action.set_group_leave(
                        self, group_id, is_dismiss
                    )
            elif self.platform["model"] in OlivOS.OPQBotLinkServerAPI.gCheckList:
                if host_id is None:
                    OlivOS.OPQBotSDK.event_action.set_group_leave(
                        self, group_id, self.plugin_info["control_queue"]
                    )
        elif self.platform["sdk"] == "telegram_poll":
    ...
    def __set_group_special_title(
        self, group_id, user_id, special_title, duration, host_id, flag_log=True
    ):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                pass
    ...
    def set_group_special_title(
        ...
        if remote:
            ...
            self.__set_group_special_title(
                group_id, user_id, special_title, duration, host_id, flag_log=True
            )
    ...
    def __set_friend_add_request(self, flag, approve, remark, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                OlivOS.onebotV12SDK.event_action.set_friend_add_request(
                    self, flag, approve, remark
                )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                OlivOS.onebotSDK.event_action.set_friend_add_request(
                    self, flag, approve, remark
                )
            elif self.platform["model"] in OlivOS.OPQBotLinkServerAPI.gCheckList:
    ...
    def __set_group_add_request(self, flag, sub_type, approve, reason, flag_log=True):
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                OlivOS.onebotV12SDK.event_action.set_group_add_request(
                    self, flag, sub_type, approve, reason
                )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                OlivOS.onebotSDK.event_action.set_group_add_request(
                    self, flag, sub_type, approve, reason
                )
            elif self.platform["model"] in OlivOS.OPQBotLinkServerAPI.gCheckList:
    ...
    def __get_login_info(self, flag_log=True):
        ...
        if flag_log and self.log_func is not None:
            if checkDictByListAnd(
                res_data, [["active"], ["data", "name"], ["data", "id"]]
            ):
                if res_data["active"]:
                    self.log_func(
                        2,
    ...
    def __get_stranger_info(self, user_id, no_cache, flag_log=True):
        ...
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                res_data = OlivOS.onebotV12SDK.event_action.get_stranger_info(
                    self, user_id
                )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                res_data = OlivOS.onebotSDK.event_action.get_stranger_info(
                    self, user_id, no_cache
                )
        elif self.platform["sdk"] == "telegram_poll":
            pass
        elif self.platform["sdk"] == "kaiheila_link":
    ...
    def __get_group_info(self, group_id, host_id, no_cache, flag_log=True):
        ...
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    res_data = OlivOS.onebotV12SDK.event_action.get_group_info(
                        self, group_id
                    )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                if host_id is None:
                    res_data = OlivOS.onebotSDK.event_action.get_group_info(
    ...
    def __get_group_list(self, flag_log=True):
        ...
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                ...
                res_data = OlivOS.OPQBotSDK.event_action.get_group_list(
                    self, self.plugin_info["control_queue"]
                )
        elif self.platform["sdk"] == "telegram_poll":
            pass
        return res_data
    ...
    def __get_group_member_info(
        self, group_id, user_id, host_id, no_cache, flag_log=True
    ):
        res_data = None
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    res_data = OlivOS.onebotV12SDK.event_action.get_group_member_info(
                        self, group_id, user_id
                    )
                else:
                    pass
                ...
                if host_id is None:
                    res_data = OlivOS.onebotSDK.event_action.get_group_member_info(
                        self, group_id, user_id, no_cache
                    )
                else:
                    res_data = OlivOS.onebotSDK.event_action.get_guild_member_profile(
                        self, host_id, user_id
                    )
        elif self.platform["sdk"] == "telegram_poll":
            res_data = OlivOS.telegramSDK.event_action.get_group_member_info(
                self, group_id, user_id
            )
        elif self.platform["sdk"] == "kaiheila_link":
            res_data = OlivOS.kaiheilaSDK.event_action.get_group_member_info(
                self, host_id, user_id
            )
        return res_data
    ...
    def get_group_member_info(
        ...
        if remote:
            ...
            res_data = self.__get_group_member_info(
                group_id, user_id, host_id, no_cache, flag_log=True
            )
        return res_data
    ...
    def __get_group_member_list(self, group_id, host_id, flag_log=True):
        ...
        if self.platform["sdk"] == "onebot":
            if self.platform["model"] in OlivOS.onebotV12LinkServerAPI.gCheckList:
                if host_id is None:
                    res_data = OlivOS.onebotV12SDK.event_action.get_group_member_list(
                        self, group_id
                    )
            elif self.platform["model"] in OlivOS.flaskServerAPI.gCheckList:
                if host_id is None:
                    res_data = OlivOS.onebotSDK.event_action.get_group_member_list(
                        self, group_id
                    )
        elif self.platform["sdk"] == "telegram_poll":
            pass
...
class Proc_templet(object):
    ...
    def start(self):
        proc_this = multiprocessing.Process(
            name=self.Proc_name, target=self.run_total, args=()
        )
...
def Proc_start(proc_this):
    proc_proc_this = multiprocessing.Process(
        name=proc_this.Proc_name, target=proc_this.run, args=()
    )
