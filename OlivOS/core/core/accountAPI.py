...
class Account(object):
    def load(path, logger_proc, safe_mode=False):
        ...
        if account_conf is None:
            logger_proc.log(
                ...
                OlivOS.L10NAPI.getTrans(
                    "init account from [{0}] ... failed", [path], modelName
                ),
            )
            ...
            logger_proc.log(
                2,
                OlivOS.L10NAPI.getTrans(
                    "init account from default ... done", [], modelName
                ),
            )
        else:
            logger_proc.log(
                2,
                OlivOS.L10NAPI.getTrans(
                    "init account from [{0}] ... done", [path], modelName
                ),
            )
        ...
        for account_conf_account_this in account_conf["account"]:
            ...
            if "extends" in account_conf_account_this and dict is type(
                account_conf_account_this["extends"]
            ):
        ...
        logger_proc.log(
            2, OlivOS.L10NAPI.getTrans("generate account ... all done", [], modelName)
        )
    ...
    def save(path, logger_proc, Account_data, safe_mode=False):
        ...
        for Account_data_this_key in Account_data:
            Account_data_this: OlivOS.API.bot_info_T = Account_data[
                Account_data_this_key
            ]
            ...
            tmp_this_account_data["platform_type"] = Account_data_this.platform[
                "platform"
            ]
        ...
        for bot_info_dict_this in bot_info_dict:
            ...
            if platform.system() == "Windows":
                if (
                    ...
                    in OlivOS.libEXEModelAPI.gCheckList
                    or Account_data_this.platform["model"]
                    in OlivOS.libNapCatEXEModelAPI.gCheckList
                ):
                ...
                if (
                    Account_data_this.platform["model"]
                    in OlivOS.libWQEXEModelAPI.gCheckList
                ):
                    if Account_data_this.post_info.auto == True:
                ...
                if (
                    Account_data_this.platform["model"]
                    in OlivOS.libCWCBEXEModelAPI.gCheckList
                ):
                    if Account_data_this.post_info.auto == True:
                        Account_data_this.post_info.type = "websocket"
                        Account_data_this.post_info.host = "ws://127.0.0.1"
                ...
                if (
                    Account_data_this.platform["model"]
                    in OlivOS.libOPQBotEXEModelAPI.gAutoCheckList
                ):
                    if Account_data_this.post_info.auto == True:
                        Account_data_this.post_info.type = "websocket"
...
class free_port_selector:
    ...
    def get_free_port(self):
        ...
        self._socket_list.append(
            s
        )  # 通过在分配端口阶段维持所有套接字，理论上可以杜绝 Issue #83
