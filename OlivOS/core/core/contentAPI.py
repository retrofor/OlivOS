...
def get_Event_from_fake_SDK(target_event):
    ...
    target_event.platform["platform"] = target_event.sdk_event.bot_info.platform[
        "platform"
    ]
    ...
    if target_event.platform["platform"] in OlivOS.messageAPI.dictMessageType:
        if (
            ...
            in OlivOS.messageAPI.dictMessageType[target_event.platform["platform"]]
        ):
            if (
...
def resourcePathTransform(ftype: str, path: str):
    ...
    if os.path.isabs(path):
        ...
        res = os.path.join(
            exePath, "data", ftype, os.path.relpath(os.path.realpath(path), exePath)
        )
    return res
