...
class logger(OlivOS.API.Proc_templet):
    def __init__(
        ...
        self.Proc_data["extend_data"] = {"std_out_handle": None}
    ...
    def run(self):
        ...
        with open(
            "%s/%s" % (logfile_dir, logfile_file_unity), "w", encoding="utf-8"
        ) as logfile_f:
            pass
        self.log(
            2,
            OlivOS.L10NAPI.getTrans(
                "OlivOS diagnose logger [{0}] is running", [self.Proc_name], modelName
            ),
        )
        ...
        while True:
            ...
            if flag_need_refresh:
                self.save_logfile()
                flag_need_refresh = False
                self.Proc_data["logfile_count_out"] = self.Proc_config[
                    "logfile_count_out"
                ]
    ...
    def save_logfile(self):
        if self.Proc_data["data_tmp"]["logfile"] != "":
            ...
            with open(
                "%s/%s" % (logfile_dir, file_name), "a+", encoding="utf-8"
            ) as logfile_f:
                try:
                    ...
                    pass
            with open(
                "%s/%s" % (logfile_dir, file_name_unity), "a+", encoding="utf-8"
            ) as logfile_f:
                try:
                    logfile_f.write(self.Proc_data["data_tmp"]["logfile"])
                except:
        ...
        if "console_color" in tmp_logger_mode_list and platform.system() == "Windows":
            ...
            if self.Proc_data["extend_data"]["std_out_handle"] is not None:
                ctypes.windll.kernel32.SetConsoleTextAttribute(
                    self.Proc_data["extend_data"]["std_out_handle"],
                    self.Proc_config["color_dict"]["shader_win"]["default"]
                    | self.Proc_config["color_dict"]["color_win"]["WHITE"]
    ...
    def log_output_shader(self, log_output_str, log_packet_this):
        ...
        if (
            log_packet_this["log_level"]
            in self.Proc_config["color_dict"]["mapping"]["need"]
        ):
            flag_have_color = True
            if (
                log_packet_this["log_level"]
                in self.Proc_config["color_dict"]["mapping"]["front"]
            ):
                tmp_color = self.Proc_config["color_dict"]["mapping"]["front"][
                    log_packet_this["log_level"]
                ]
            if (
                log_packet_this["log_level"]
                in self.Proc_config["color_dict"]["mapping"]["background"]
            ):
                tmp_color_bak = self.Proc_config["color_dict"]["mapping"]["background"][
                    log_packet_this["log_level"]
                ]
            if (
                log_packet_this["log_level"]
                in self.Proc_config["color_dict"]["mapping"]["shader"]
            ):
                tmp_shader = self.Proc_config["color_dict"]["mapping"]["shader"][
                    log_packet_this["log_level"]
                ]
        if platform.system() == "Windows":
            if (
                flag_have_color
                and self.Proc_data["extend_data"]["std_out_handle"] is not None
            ):
                ctypes.windll.kernel32.SetConsoleTextAttribute(
                    self.Proc_data["extend_data"]["std_out_handle"],
        ...
        if log_packet_this["log_level"] in self.Proc_config["logger_vis_level"]:
            ...
            for tmp_logger_mode_list_this in tmp_logger_mode_list:
                if tmp_logger_mode_list_this in [
                    ...
                    log_output_str += (
                        "["
                        + self.Proc_config["level_dict"][log_packet_this["log_level"]]
                        + "]"
                        + " - "
                    )
                    ...
                    if tmp_logger_mode_list_this == "console_color":
                        ...
                        if flag_need_refresh:
                            self.save_logfile()
        if (
            type(self.Proc_config["logger_mode"]) == list
    ...
    def __get_log_message(self, log_packet_this):
        ...
        for segment_this in log_packet_this["log_segment"]:
            ...
            log_output_str_1 += (
                self.Proc_config["segment_type"][segment_this_type][1] + " - "
            )
