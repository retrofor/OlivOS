...
class shallow(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        time.sleep(
            1
        )  # 此处延迟用于在终端第一次启动时等待终端初始化，避免日志丢失，后续需要用异步(控制包流程)方案替代
        ...
        self.log(
            ...
            OlivOS.L10NAPI.getTrans(
                "OlivOS plugin shallow [{0}] is running", [self.Proc_name], modelName
            ),
        )
        self.sendPluginList()
        rx_count = 0
        while True:
            if self.Proc_info.rx_queue.empty() or self.Proc_config["ready_for_restart"]:
                ...
                if type(rx_packet_data) == OlivOS.API.Control.packet:
                    if (
                        rx_packet_data.action == "restart_do"
                        and self.Proc_config["enable_auto_restart"]
                    ):
                        self.Proc_config["ready_for_restart"] = True
                        self.run_plugin_func(None, "save")
                        self.Proc_info.control_queue.put(
                            OlivOS.API.Control.packet("restart_do", self.Proc_name),
                            block=False,
                        )
                        self.log(
                            2,
                            OlivOS.L10NAPI.getTrans(
                                "OlivOS plugin shallow [{0}] will restart",
                                [self.Proc_name],
                                modelName,
                            ),
                        )
                        ...
                        rx_packet_data.action == "update_hit"
                        and self.Proc_config["enable_auto_restart"]
                    ):
                        self.Proc_config["ready_for_restart"] = True
                        self.run_plugin_func(None, "save")
                        self.Proc_info.control_queue.put(
                            OlivOS.API.Control.packet("init_type", "update_replace"),
                            block=False,
                        )
                        # 在运行过 save 指令后，将配置数据库关闭
                        self.database.stop()
                    ...
                    if self.Proc_config["treading_mode"] == "none":
                        ...
                        t_run_plugin = None
                        t_run_plugin = threading.Thread(
                            target=self.run_plugin, args=(rx_packet_data.sdk_event,)
                        )
                        t_run_plugin.start()
    ...
    def set_restart(self):
        self.log(
            2,
            OlivOS.L10NAPI.getTrans(
                "OlivOS plugin shallow [{0}] call restart", [self.Proc_name], modelName
            ),
        )
    ...
    def run_plugin(self, sdk_event):
        ...
        if plugin_event.active:
            if plugin_event_bot_hash in self.Proc_data["bot_info_dict"]:
                plugin_event.bot_info = self.Proc_data["bot_info_dict"][
        ...
        if plugin_event.active:
            ...
            for plugin_models_index_this in self.plugin_models_call_list:
                ...
                for plugin_model_support_this in self.plugin_models_dict[
                    ...
                    if plugin_event.platform["sdk"] == "all":
                        ...
                        plugin_model_support_this["sdk"] == plugin_event.platform["sdk"]
                    ):
                        flag_support_found_dict["sdk"] = True
                    if plugin_event.platform["platform"] == "all":
                        ...
                        plugin_model_support_this["platform"]
                    ...
                    if plugin_event.platform["model"] == "all":
                        ...
                        plugin_model_support_this["model"]
                        == plugin_event.platform["model"]
                    ):
                        flag_support_found_dict["model"] = True
                    if (
                ...
                if flag_support_found_dict["flag"]:
                    plugin_event.plugin_info["name"] = self.plugin_models_dict[
                        plugin_models_index_this
                    ]["name"]
                    plugin_event.plugin_info["namespace"] = self.plugin_models_dict[
                        plugin_models_index_this
                    ]["namespace"]
                    if (
                        "message_mode"
    ...
    def load_plugin_list(self):
                ...
                if len(plugin_dir_this_tmp) > 4:
                    if plugin_dir_this_tmp[-4:] == ".opk":
                        ...
                        for opkFile_list_this in opkFile_list:
                            opkFile.extract(
                                opkFile_list_this, plugin_path_tmp + plugin_dir_this
                            )
            except Exception as e:
                doOpkRemove(plugin_path_tmp, plugin_dir_this_tmp)
                # traceback.print_exc()
                ...
                if len(plugin_dir_this) > 0:
                    if plugin_dir_this[0] not in ["."]:
                        try:
                            if flag_is_opk:
                                with open(
                                    plugin_path_tmp + plugin_dir_this + "/app.json",
                                    "r",
                                    encoding="utf-8",
                                ) as plugin_models_app_conf_f:
                                    plugin_models_app_conf = json.loads(
                                        plugin_models_app_conf_f.read()
                                    )
                            else:
                                with open(
                                    plugin_path + plugin_dir_this + "/app.json",
                                    "r",
                                    encoding="utf-8",
                                ) as plugin_models_app_conf_f:
                                    plugin_models_app_conf = json.loads(
                        ...
                        if plugin_models_app_conf is None:
                            ...
                            if "namespace" in plugin_models_app_conf:
                                plugin_models_dict_this["namespace"] = (
                                    plugin_models_app_conf["namespace"]
                                )
                            if "menu_config" in plugin_models_app_conf:
                                plugin_models_dict_this["menu_config"] = (
                                    plugin_models_app_conf["menu_config"]
                                )
                            if "message_mode" in plugin_models_app_conf:
                                plugin_models_dict_this["message_mode"] = (
                                    plugin_models_app_conf["message_mode"]
                                )
                            else:
                                plugin_models_dict_this["message_mode"] = (
                                    OlivOS.infoAPI.OlivOS_message_mode_tx_default
                            ...
                            if "author" in plugin_models_app_conf:
                                plugin_models_dict_this["author"] = (
                                    plugin_models_app_conf["author"]
                                )
                            else:
                                plugin_models_dict_this["author"] = "N/A"
                            if "svn" in plugin_models_app_conf:
                                plugin_models_dict_this["svn"] = plugin_models_app_conf[
                                    "svn"
                                ]
                            else:
                                plugin_models_dict_this["svn"] = 0
                            if "version" in plugin_models_app_conf:
                                plugin_models_dict_this["version"] = (
                                    plugin_models_app_conf["version"]
                                )
                            else:
                                plugin_models_dict_this["version"] = "0"
                            if "info" in plugin_models_app_conf:
                                plugin_models_dict_this["info"] = (
                                    plugin_models_app_conf["info"]
                                )
                            else:
                                plugin_models_dict_this["info"] = "N/A"
                            if "compatible_svn" in plugin_models_app_conf:
                                plugin_models_dict_this["compatible_svn"] = (
                                    plugin_models_app_conf["compatible_svn"]
                                )
                            else:
                                plugin_models_dict_this["compatible_svn"] = (
                                    OlivOS.infoAPI.OlivOS_compatible_svn_default
                                )
                            if (
                                plugin_models_dict_this["compatible_svn"]
                                >= OlivOS.infoAPI.OlivOS_SVN_Compatible
                            ):
                                pass
                            elif (
                                plugin_models_dict_this["compatible_svn"]
                                >= OlivOS.infoAPI.OlivOS_SVN_OldCompatible
                                and plugin_models_dict_this["compatible_svn"]
                                <= OlivOS.infoAPI.OlivOS_SVN_Compatible
                            ):
                                self.log(
                                    3,
        ...
        for plugin_models_dict_this_key in plugin_models_dict:
            try:
                # 从这里开始导入实际模块
                plugin_models_dict_this = plugin_models_dict[
                    plugin_models_dict_this_key
                ]["data"]
                ...
                if hasattr(plugin_models_tmp, "main"):
                    if hasattr(plugin_models_tmp.main, "Event"):
                        self.plugin_models_dict[plugin_dir_this] = (
                            plugin_models_dict_this
                        )
                        if hasattr(plugin_models_tmp.main.Event, func_init_name):
                            try:
                                plugin_models_tmp.main.Event.init(
                                    plugin_event=None, Proc=self
                                )
                                self.log(
                                    2,
                                    OlivOS.L10NAPI.getTrans(
                        ...
                        # doOpkRemove(plugin_path_tmp, plugin_dir_this_tmp)
                        self.run_plugin_data_release_by_name(
                            plugin_models_dict_this_key
                        )
                        continue
                    else:
        ...
        plugin_models_call_list_tmp = sorted(
            self.plugin_models_dict.values(),
