...
def OlivOSUpdateReplace(logger_proc):
    if platform.system() == "Windows":
        ...
        with open(update_bat_name, "w") as b:
            ...
            b.write(TempList)
        subprocess.call(
            'start cmd /b /K ".\\' + update_bat_name + '"', shell=True, cwd=".\\"
        )
        OlivOS.bootAPI.killMain()


def OlivOSUpdateGet(logger_proc, flagChackOnly=False, control_queue=None):
    res = False
    ...
    if platform.system() == "Windows":
        ...
        if exe_name is not None:
            ...
            logger_proc.log(
                2, OlivOS.L10NAPI.getTrans("check OlivOS update ......", [], modelName)
            )
            ...
            if down_url_obj is not None:
                try:
                    if (
                        type(down_url_obj["version"]["OlivOS"][architecture_num]["svn"])
                        == int
                        and down_url_obj["version"]["OlivOS"][architecture_num]["svn"]
                        > OlivOS.infoAPI.OlivOS_SVN
                    ):
                        down_url = down_url_obj["version"]["OlivOS"][architecture_num][
                            "path"
                        ]
                        if flagChackOnly:
                            logger_proc.log(
                                ...
                                OlivOS.L10NAPI.getTrans(
                                    "OlivOS update found, please try update.",
                                    [],
                                    modelName,
                                ),
                            )
                        ...
                        logger_proc.log(
                            2,
                            OlivOS.L10NAPI.getTrans(
                                "OlivOS already latest.", [], modelName
                            ),
                        )
                except:
                    down_url = None
                    logger_proc.log(
                        3,
                        OlivOS.L10NAPI.getTrans(
                            "check OlivOS update api error, skip update replace.",
                            [],
                            modelName,
                        ),
                    )
            ...
            if not flagChackOnly and down_url is not None:
                if GETHttpFile(down_url, down_file_name):
                    ...
                    if os.path.isfile(target_name):
                        ...
                        logger_proc.log(
                            2,
                            OlivOS.L10NAPI.getTrans(
                                "check OlivOS update file hit, will run update replace.",
                                [],
                                modelName,
                            ),
                        )
                        ...
                        logger_proc.log(
                            3,
                            OlivOS.L10NAPI.getTrans(
                                "check OlivOS update file not hit, skip update replace.",
                                [],
                                modelName,
                            ),
                        )
...
def checkResouceFile(
    ...
    logger(
        2,
        OlivOS.L10NAPI.getTrans(
            "will check {0} lib after {1}s ...", [resouce_name, sleepTime], modelName
        ),
    )
    ...
    for i in range(1):
        ...
        logger(
            ...
            OlivOS.L10NAPI.getTrans(
                ...
                [resouce_name, filePath, str(fMD5)],
                modelName,
            ),
        )
        ...
        if not flagFORCESKIP:
            ...
            fMD5UpdateUrl = None
            try:
                fMD5UpdateTarget = apiJsonData["version"][resouce_name][
                    architecture_num
                ]["md5"]
                fMD5UpdateUrl = apiJsonData["version"][resouce_name][architecture_num][
                    "path"
                ]
            except:
                fMD5UpdateTarget = None
            if (
                ...
                logger(
                    ...
                    OlivOS.L10NAPI.getTrans(
                        ...
                        [resouce_name, str(fMD5UpdateTarget)],
                        modelName,
                    ),
                )
                if fMD5UpdateTarget != fMD5:
                    logger(
                        2,
                        OlivOS.L10NAPI.getTrans(
                            "download new {0} lib ...", [resouce_name], modelName
                        ),
                    )
                    if OlivOS.updateAPI.GETHttpFile(fMD5UpdateUrl, filePathUpdate):
                        logger(
                            2,
                            OlivOS.L10NAPI.getTrans(
                                "download new {0} lib done", [resouce_name], modelName
                            ),
                        )
                        ...
                        logger(
                            ...
                            OlivOS.L10NAPI.getTrans(
                                "check {0} lib patch [{1}] md5: [{2}]",
                                [resouce_name, filePathUpdate, str(fMD5Update)],
                                modelName,
                            ),
                        )
                    else:
                        ...
                        logger(
                            4,
                            OlivOS.L10NAPI.getTrans(
                                "download new {0} lib FAILED! md5 check FAILED!",
                                [resouce_name],
                                modelName,
                            ),
                        )
                ...
                logger(
                    4,
                    OlivOS.L10NAPI.getTrans(
                        "download {0} lib patch FAILED! try later please!",
                        [resouce_name],
                        modelName,
                    ),
                )
            ...
            if flagAlreadyLatest:
                logger(
                    2,
                    OlivOS.L10NAPI.getTrans(
                        "{0} lib already latest!", [resouce_name], modelName
                    ),
                )
                ...
                logger(
                    3,
                    OlivOS.L10NAPI.getTrans(
                        "update {0} lib patch [{1}] -> [{2}]",
                        [resouce_name, filePathUpdate, filePath],
                        modelName,
                    ),
                )
                ...
                logger(
                    2,
                    OlivOS.L10NAPI.getTrans(
                        "update {0} lib patch done!", [resouce_name], modelName
                    ),
                )
            else:
                logger(
            ...
            logger(
                3,
                OlivOS.L10NAPI.getTrans(
                    "{0} lib update FORCESKIP!", [resouce_name], modelName
                ),
            )
        break
...
def loggerGen(logger_proc: "OlivOS.diagnoseAPI.logger|None"):
    def logF(log_level, log_message, log_segment=None):
        if type(logger_proc) is OlivOS.diagnoseAPI.logger:
            logger_proc.log(
                log_level=log_level, log_message=log_message, log_segment=log_segment
            )
