...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        while self.flag_run:
            self.sendLogSim(
                2,
                "OlivOS libAstralQsignEXEModel server [{0}] will run under visiable mode",
                [self.Proc_name],
            )
            time.sleep(2)
            ...
            self.sendLog(
                2,
                "OlivOS libAstralQsignEXEModel server [{0}] is running on port [{1}]",
    ...
    def setGoCqhttpModelEnableSendAll(self):
        for bot_info_key in self.Proc_data["bot_info_dict"]:
            if (
                self.Proc_data["bot_info_dict"][bot_info_key].platform["model"]
                in gCheckList
            ):
                self.setGoCqhttpModelEnableSend(
                    self.Proc_data["bot_info_dict"][bot_info_key].hash
                )

    def sendLog(self, log_level: int, log_message: str, log_message_list: list):
        self.log(
            log_level,
            OlivOS.L10NAPI.getTrans(log_message, log_message_list, modelName),
            [("AstralQsign", "default")],
        )
...
def isBotActive(plugin_bot_info_dict: dict):
    ...
    for bot_info_key in plugin_bot_info_dict:
        if (
            ...
            and plugin_bot_info_dict[bot_info_key].platform["model"]
            in OlivOS.libAstralQsignEXEModelAPI.gCheckList
            and plugin_bot_info_dict[bot_info_key].extends.get(
                "qsign-server-protocal", None
            )
            == "AstralQsign"
        ):
...
def unzip(zip_file_path, target_dir):
    with support_gbk(
        zipfile.ZipFile(zip_file_path, "r", zipfile.ZIP_DEFLATED)
    ) as zip_file:
