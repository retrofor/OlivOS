...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        if self.Proc_data["bot_info_dict"].platform["model"] in ["ComWeChatBotClient"]:
        ...
        while self.flag_run:
            ...
            releaseDir(
                "./conf/ComWeChatBotClient/" + self.Proc_data["bot_info_dict"].hash
            )
            ...
            if self.Proc_data["bot_info_dict"].platform["model"] in [
                ...
                model_Proc = subprocess.Popen(
                    ...
                    cwd=".\\conf\\ComWeChatBotClient\\"
                    + self.Proc_data["bot_info_dict"].hash,
                    shell=True,
                    stdin=subprocess.PIPE,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE,
        ...
        for line in iter(model_Proc.stdout.readline, b""):
            try:
                ...
                self.log(
                    4,
                    OlivOS.L10NAPI.getTrans(
                        "OlivOS libCWCBEXEModel failed: %s\n%s"
...
"""
        ...
        with open(
            "./conf/ComWeChatBotClient/" + self.bot_info_dict.hash + "/.env",
            "w+",
            encoding="utf-8",
        ) as tmp:
