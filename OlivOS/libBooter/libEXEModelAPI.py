            ...
            if plugin_bot_info_dict[bot_info_key].platform["model"] in gCheckList:
                ...
                Proc_dict[tmp_Proc_name] = OlivOS.libEXEModelAPI.server(
                    ...
                    target_proc=basic_conf_models[
                        basic_conf_models_this["target_proc"]
                    ],
                    sub_target_proc=basic_conf_models.get(
                        basic_conf_models_this.get("sub_target_proc", None), None
                    ),
                    debug_mode=False,
                )
...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        while self.flag_run:
            ...
            if (
                ...
                and type(
                    self.Proc_data["bot_info_dict"].extends["qsign-server-protocal"]
                )
                is str
                and self.Proc_data["bot_info_dict"].extends["qsign-server-protocal"]
                ...
                self.log(
                    ...
                    OlivOS.L10NAPI.getTrans(
                        ...
                        [self.Proc_name, str(8)],
                        modelName,
                    ),
                )
                self.check_protocalEXE(8, 0.25)
            if False and (
                self.Proc_data["bot_info_dict"].platform["model"]
                in ["gocqhttp", "gocqhttp_hide"]
            ):
                ...
                threading.Thread(
                    target=self.check_qrcode, args=(), daemon=self.deamon
                ).start()
                ...
                threading.Thread(
                    target=self.check_stdin, args=(model_Proc,), daemon=self.deamon
                ).start()
    ...
    def clear_gocqhttp(self):
        file_path = (
            "./conf/gocqhttp/" + self.Proc_data["bot_info_dict"].hash + "/qrcode.png"
        )
...
"""
        ...
        self.config_file_format["access-token"] = (
            self.bot_info_dict.post_info.access_token
        )
...
"""
        ...
        with open(
            "./conf/gocqhttp/" + self.bot_info_dict.hash + "/config.yml",
            "w+",
            encoding="utf-8",
        ) as tmp:
...
def accountFix(bot_info_dict, logger_proc):
    ...
    for bot_info_dict_this in bot_info_dict:
        ...
        if (
            ...
            # 协议修改
            if (
                bot_info_dict[bot_hash].platform["model"]
                == "gocqhttp_show_Android_Phone"
            ):
                ...
                bot_info_dict[bot_hash].platform["model"] == "gocqhttp_show_Android_Pad"
            ):
            ...
            if (
                ...
                and type(bot_info_dict[bot_hash].extends["qsign-server-protocal"])
                is str
            ):
                if bot_info_dict[bot_hash].platform["model"] in [
                    ...
                    if bot_info_dict[bot_hash].platform["model"] in [
                        ...
                        if (
                            bot_info_dict[bot_hash].extends["qsign-server-protocal"]
                            in gProtocalInfo["android_phone"]
                        ):
            ...
            if protocal_info is not None:
                file_path = (
                    "./conf/gocqhttp/"
                    + bot_hash
                    + "/data/versions/%d.json" % protocal_num
                )
                try:
                    with open(file_path, "w", encoding="utf-8") as f:
...
def deviceInfoFix(deviceInfo: dict):
    ...
    if flagRelease:
        ...
        deviceResPatch["finger_print"] = (
            ...
            % getRandomStringOfInt(7)
        )
        deviceResPatch["boot_id"] = str(uuid.uuid4())
        deviceResPatch["proc_version"] = (
            "Linux version 3.0.31-%s (android-build@xxx.xxx.xxx.xxx.com)"
            % getRandomString(8)
        )
...
def getRandomString(length: int):
    return getRandomStringRange(
        length, "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890"
    )
