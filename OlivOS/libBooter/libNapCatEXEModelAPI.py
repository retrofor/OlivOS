...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        while self.flag_run:
            ...
            if self.Proc_data["bot_info_dict"].platform["model"] in ["napcat_show_new"]:
                unzip(
                    "./lib/NapCatNew.zip",
            ...
            napcatTypeConfig(
                self.Proc_data["bot_info_dict"], self.Proc_config["target_proc"]
            ).setConfig()
            if self.Proc_data["bot_info_dict"].platform["model"] in [
                "napcat",
                "napcat_show_new",
...
class napcatTypeConfig(object):
    ...
    def setConfig(self):
        ...
        self.config_file_str = json.dumps(
            self.config_file_data, ensure_ascii=False, indent=4
        )
