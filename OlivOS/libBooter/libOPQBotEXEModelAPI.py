...
gCheckList = ["opqbot_auto", "opqbot_port", "opqbot_port_old"]

gAutoCheckList = ["opqbot_auto"]
...
class server(OlivOS.API.Proc_templet):
    ...
    def run(self):
        ...
        while self.flag_run:
            ...
            if self.Proc_data["bot_info_dict"].platform["model"] in [
                ...
                threading.Thread(
                    target=self.check_model_flag, args=(), daemon=self.deamon
                ).start()
        ...
        for line in iter(model_Proc.stdout.readline, b""):
            try:
                ...
                self.log(
                    4,
                    OlivOS.L10NAPI.getTrans(
                        "OlivOS libOPQBotEXEModel failed: %s\n%s"
    ...
    def clear_OPQBot(self):
        file_path = (
            "./conf/OPQBot/" + self.Proc_data["bot_info_dict"].hash + "/qrcode.png"
        )
