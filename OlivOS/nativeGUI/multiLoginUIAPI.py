    ...
    if control_queue is not None:
        control_queue.put(
            OlivOS.API.Control.packet(
                "call_system_event", {"action": ["account_edit_asayc_end"]}
            ),
            block=False,
        )
        control_queue.put(
            OlivOS.API.Control.packet("call_account_update", {"data": account_data}),
            block=False,
        )
        control_queue.put(
            OlivOS.API.Control.packet(
                "call_system_stop_type_event", {"action": ["account_update"]}
            ),
            block=False,
        )
        control_queue.put(
            OlivOS.API.Control.packet(
                "call_system_event", {"action": ["account_update"]}
            ),
            block=False,
        )
...
class HostUI(object):
    ...
    def start(self):
        ...
        self.UIObject["root"].title(
            "OlivOS 登录管理器 - %s" % OlivOS.infoAPI.OlivOS_Version_Title
        )
        ...
        self.UIObject["tree_rightkey_menu"] = tkinter.Menu(
            self.UIObject["root"], tearoff=False
        )
        ...
        self.UIObject["tree_yscroll"].place(x=500, y=0, width=18, height=350)
        ...
        self.UIObject["root_frame_first_root"].configure(
        ...
        self.UIObject["root_frame_first_root_label_note_new"] = tkinter.Label(
            ...
            text="\n".join(["欢迎使用 OlivOS", "", "你可以在这里创建你的第一个账号"]),
        )
        ...
        self.UIObject["root_frame_first_root_Button_FIRST_NEW"].configure(
            font="等线 16 bold"
        )

        self.UIObject["root_frame_first_root_label_note_commit"] = tkinter.Label(
            self.UIObject["root_frame_first_root"], text="\n".join(["或者你也可以"])
        )
        ...
        self.UIObject["root_frame_first_root_label_note_commit"].place(
            x=391 - 16 * 6 - 8, y=358, width=16 * 6, height=34
        )
        ...
        self.UIObject["root_frame_skip_root"].configure(
            bg=self.UIConfig["color_001"], borderwidth=0
        )
        # 这个组件使用这两个方法进行显示和隐藏
        # 位置信息依赖此处的数据
        ...
        self.UIObject["root_frame_skip_root_Button_FIRST_COMMIT"].configure(
            font="等线 16 bold"
        )
        ...
        self.UIObject["root_frame_skip_root_Button_FIRST_COMMIT_BACK"].configure(
            font="等线 16 bold"
        )
    ...
    def tree_UI_Button_init(
        self, name, text, command, x, y, width, height, root="root"
    ):
        ...
        self.UIObject[name].bind(
            "<Enter>", lambda x: self.buttom_action(name, "<Enter>")
        )
        self.UIObject[name].bind(
            "<Leave>", lambda x: self.buttom_action(name, "<Leave>")
        )
    ...
    def tree_rightKey(self, event):
        ...
        self.UIObject["tree_rightkey_menu"].add_command(
            label="新建", command=lambda: self.tree_edit("create")
        )
        self.UIObject["tree_rightkey_menu"].add_command(
            label="编辑", command=lambda: self.tree_edit("update")
        )
        self.UIObject["tree_rightkey_menu"].add_command(
            label="删除", command=lambda: self.tree_edit("delete")
        )
        self.UIObject["tree_rightkey_menu"].add_command(
            label="确认", command=lambda: self.account_data_commit()
        )
    ...
    def tree_edit_commit(self, Edit_res):
        [Edit_action, Edit_old_hash, Edit_new_info] = Edit_res
        ...
        for type_this in OlivOS.accountMetadataAPI.accountTypeList:
            ...
            for list_data_check_i in range(len(list_data_check)):
                if (
                    ...
                    != OlivOS.accountMetadataAPI.accountTypeMappingList[type_this][
                        list_data_check_i
                    ]
                ):
                    flag_hit = False
...
class TreeEditUI(object):
    def __init__(self, action, Account_data, hash_key=None, edit_commit_callback=None):
        ...
        self.UIData = {
            ...
            "edit_root_Combobox_qsign_protocal_list_exemod": ["AstralQsign"],
            ...
            "edit_root_Combobox_dict": {
                ...
                "type_mapping_list_Entry_slot": {
                    ...
                    "微信/ComWeChat": {"微信号": "edit_root_Entry_ID"},
                    "KOOK": {"Token": "edit_root_Entry_Server_access_token"},
                    "KOOK/消息兼容": {"Token": "edit_root_Entry_Server_access_token"},
                    "米游社/大别野/公域": {
                    ...
                    "Telegram": {"TOKEN": "edit_root_Entry_Server_access_token"},
                    "Discord": {"TOKEN": "edit_root_Entry_Server_access_token"},
                    "Discord/指定intents": {
                        "TOKEN": "edit_root_Entry_Server_access_token",
                        "intents": "edit_root_Entry_Server_port",
                    },
                    ...
                    "Fanbook": {"Token": "edit_root_Entry_Server_access_token"},
                    "Hack.Chat": {
                    ...
                    "钉钉": {"Robot Code": "edit_root_Entry_ID"},
                    "自定义": {
        ...
        for key_this in OlivOS.accountMetadataAPI.accountTypeMappingList:
            tmp_mapping_slot = copy.deepcopy(
                OlivOS.accountMetadataAPI.accountTypeMappingList[key_this]
            )
            tmp_Entry_slot = None
            if (
                key_this
                in self.UIData["edit_root_Combobox_dict"][
                    "type_mapping_list_Entry_slot"
                ]
            ):
                tmp_Entry_slot = self.UIData["edit_root_Combobox_dict"][
                    "type_mapping_list_Entry_slot"
                ][key_this]
            else:
                tmp_Entry_slot = {}
    ...
    def tree_edit_commit(self):
        ...
        if self.action == "create" or self.action == "update":
            ...
            tmp_server_auto = self.UIData[
                "edit_root_Combobox_Server_auto_StringVar"
            ].get()
            tmp_server_type = self.UIData[
                "edit_root_Combobox_Server_type_StringVar"
            ].get()
            ...
            tmp_access_token = self.UIData[
                "edit_root_Entry_Server_access_token_StringVar"
            ].get()
            tmp_platform_sdk = self.UIData["edit_root_Combobox_sdk_StringVar"].get()
            tmp_platform_platform = self.UIData[
                "edit_root_Combobox_platform_StringVar"
            ].get()
            ...
            tmp_extends = [
                ...
                for edit_root_Entry_Extend_this in self.UIData[
                    "edit_root_Entry_Extend_list"
                ]
            ]
            if (
            ...
            if (
                ...
                if tmp_host == "":
                    tmp_host = "127.0.0.1"
                if tmp_platform_model in ["opqbot_auto"]:
                    if tmp_port == "":
                        tmp_port = "8086"
            if (
            ...
            if (
                ...
                if tmp_host == "":
                    tmp_host = "NONEED"
                if tmp_platform_model not in ["intents"]:
                    if tmp_port == "":
                        tmp_port = "0"
            if (
            ...
            if not checkByListEmptyOr([
                ...
                if (
                    ...
                    and type_this
                    in self.UIData["edit_root_Combobox_dict"]["type_extends_note_list"]
                    and type_this
                    in self.UIData["edit_root_Combobox_dict"][
                        "type_extends_name_note_list"
                    ]
                    ...
                    tmp_offset = 0
                    for tmp_Entry_this in self.UIData["edit_root_Combobox_dict"][
                        "type_extends_name_note_list"
                    ][type_this]:
                ...
                if (
                    ...
                    tmp_res_bot_info.extends["qsign-server-protocal"] = self.UIData[
                        "edit_root_Combobox_qsign_protocal_StringVar"
                    ].get()
                    ...
                    for tmp_i in range(self.UIData["edit_root_Entry_qsign_num"]):
                        ...
                        if key_pare[0] + "_StringVar" in self.UIData:
                            tmp_data["addr"] = self.UIData[
                                key_pare[0] + "_StringVar"
                            ].get()
                        if key_pare[1] + "_StringVar" in self.UIData:
                            tmp_data["key"] = self.UIData[
                                key_pare[1] + "_StringVar"
                            ].get()
                        if len(tmp_data["addr"]) > 0 or len(tmp_data["key"]) > 0:
                            tmp_res_bot_info.extends["qsign-server"].append(tmp_data)
                self.UIData["Edit_res"] = [tmp_action, self.hash_key, tmp_res_bot_info]
            else:
                miss_key_list = []
                tmp_check_list = [
                    ["ID", tmp_id],
            ...
            self.UIData["Edit_res"] = ["delete", self.hash_key, None]
    ...
    def tree_edit_UI(self):
        ...
        self.UIObject["edit_root"].resizable(width=False, height=False)
    ...
    def tree_edit_UI_Label_init(
        self, obj_root, obj_name, x, y, width, height, title=""
    ):
        self.UIObject[obj_name] = tkinter.Label(self.UIObject[obj_root], text=title)
    ...
    def tree_edit_UI_Entry_init(
        ...
        self.UIObject[obj_name + "=Label"] = tkinter.Label(
            self.UIObject[obj_root], text=title
        )
        self.UIObject[obj_name + "=Label"].configure(
            bg=self.UIConfig["color_001"], fg=self.UIConfig["color_004"]
        )
        self.UIObject[obj_name + "=Label"].place(
            x=x - 100, y=y, width=100, height=height
        )
        self.UIObject[obj_name] = tkinter.Entry(
            self.UIObject[obj_root], textvariable=self.UIData[str_name]
        )
        self.UIObject[obj_name].configure(
            bg=self.UIConfig["color_004"], fg=self.UIConfig["color_005"], bd=0
        )
        if mode == "SAFE":
            self.UIObject[obj_name].configure(show="●")
        self.UIObject[obj_name].place(x=x, y=y, width=width, height=height)

    def tree_edit_UI_Combobox_init(
        self, obj_root, obj_name, str_name, x, y, width, height, action, title=""
    ):
        self.UIObject[obj_name + "=Label"] = tkinter.Label(
            self.UIObject[obj_root], text=title
        )
    ...
    def tree_edit_UI_type_clear_note_GEN(self, tmp_type: str):
        def tree_edit_UI_type_clear_note():
            if (
                tmp_type
                in self.UIData["edit_root_Combobox_dict"]["type_clear_note_list"]
            ):
                valDict = {"bothash": self.hash_key}
                dirPath = self.UIData["edit_root_Combobox_dict"][
                    "type_clear_note_list"
                ][tmp_type].format(**valDict)
                removeDir(dirPath)
    ...
    def tree_edit_UI_Combobox_update(self, action, con_action):
        ...
        if con_action == "init":
            self.UIObject["edit_root_Combobox_Account_type"]["value"] = tuple(
                tmp_list_type
            )
            if action == "create":
            ...
            if action == "update":
                if self.hash_key in self.UIData["Account_data"]:
                    Account_data_this: OlivOS.API.bot_info_T = self.UIData[
                        "Account_data"
                    ][self.hash_key]
                tmp_list_match_old = self.UIData["edit_root_Combobox_dict"][
                    "type_mapping_list"
                ]
                tmp_list_match = {}
                tmp_list_data = [
                ...
                for index in [0, 1, 2, 3, 4]:
                    ...
                    for tmp_list_match_old_this in tmp_list_match_old:
                        if tmp_list_match_old_this != "自定义" and str(
                            tmp_list_data[index]
                        ) == str(tmp_list_match_old[tmp_list_match_old_this][index]):
                            tmp_list_match[tmp_list_match_old_this] = (
                                tmp_list_match_old[tmp_list_match_old_this]
                            )
                ...
                if len(tmp_list_match) > 0:
                    ...
                    self.UIObject["edit_root_Combobox_Account_type"].current(
                        self.UIData["edit_root_Combobox_dict"]["type_list"].index(
                            tmp_data_match
                        )
                    )
                else:
            ...
            if tmp_type in self.UIData["edit_root_Combobox_dict"]["type_mapping_list"]:
                ...
                if tmp_type != "自定义":
                    ...
                    for entry_this in [
                        ...
                        count += 1
                for entry_this in self.UIData["edit_root_Combobox_dict"][
                    "type_mapping_list"
                ][tmp_type][5]:
                    tmp_mode = "NONE"
                    if (
                    ...
                    self.tree_edit_UI_Entry_init(
                        obj_root="edit_root",
                        obj_name=self.UIData["edit_root_Combobox_dict"][
                            "type_mapping_list"
                        ][tmp_type][5][entry_this],
                        str_name=self.UIData["edit_root_Combobox_dict"][
                            "type_mapping_list"
                        ][tmp_type][5][entry_this]
                        + "_StringVar",
                        x=100,
                    ...
                    self.UIObject["edit_root"].geometry(
                        "400x%s" % (count * (24 + 6) + 100 + 10)
                    )
                    count += 1
                if (
                    tmp_type
                    ...
                    tmp_offset = 0
                    self.UIData["edit_root_Combobox_dict"]["type_extends_note_list"][
                        tmp_type
                    ]
                    for Entry_Extend_this in self.UIData["edit_root_Combobox_dict"][
                        "type_extends_name_note_list"
                    ][tmp_type]:
                        if (
                            Entry_Extend_this
                            ...
                            if tmp_offset >= len(
                                self.UIData["edit_root_Entry_Extend_list"]
                            ):
                                break
                            self.tree_edit_UI_Entry_init(
                                obj_root="edit_root",
                                obj_name=self.UIData["edit_root_Entry_Extend_list"][
                                    tmp_offset
                                ],
                                str_name=self.UIData["edit_root_Entry_Extend_list"][
                                    tmp_offset
                                ]
                                + "_StringVar",
                                x=100,
                ...
                if tmp_type in self.UIData["edit_root_Combobox_dict"]["type_note_list"]:
                    self.tree_edit_UI_Label_init(
                        ...
                        width=400 - 15 * 2,
                        height=24,
                        title=self.UIData["edit_root_Combobox_dict"]["type_note_list"][
                            tmp_type
                        ],
                    )
                    self.UIObject["edit_root"].geometry(
                ...
                if (
                    ...
                    tmp_qsign_protocal = self.UIData[
                        "edit_root_Combobox_qsign_protocal_StringVar"
                    ].get()
                    ...
                    if con_action == "init":
                        if action == "update":
                            if self.hash_key in self.UIData["Account_data"]:
                                Account_data_this = self.UIData["Account_data"][
                                    self.hash_key
                                ]
                                if (
                                    "qsign-server-protocal" in Account_data_this.extends
                                    and type(
                                        Account_data_this.extends[
                                            "qsign-server-protocal"
                                        ]
                                    )
                                    is str
                                    and Account_data_this.extends[
                                        "qsign-server-protocal"
                                    ]
                                    in self.UIData[
                                        "edit_root_Combobox_qsign_protocal_list"
                    ...
                    if (
                        tmp_qsign_protocal
                        in self.UIData["edit_root_Combobox_qsign_protocal_list_exemod"]
                    ):
                        tmp_note_name = "edit_root_Label_qsign_note_astalqsign_will_set"
                        if (
                            tmp_note_name
                            not in self.UIData["edit_root_Entry_qsign_list"]
                        ):
                            self.UIData["edit_root_Entry_qsign_list"].append(
                                tmp_note_name
                            )
                        self.tree_edit_UI_Label_init(
                            obj_root="edit_root",
                        ...
                        for tmp_i in range(array_num):
                            ...
                            if key_pare[0] + "_StringVar" not in self.UIData:
                                self.UIData[key_pare[0] + "_StringVar"] = (
                                    tkinter.StringVar()
                                )
                            self.tree_edit_UI_Entry_init(
                                obj_root="edit_root",
                            ...
                            if key_pare[1] + "_StringVar" not in self.UIData:
                                self.UIData[key_pare[1] + "_StringVar"] = (
                                    tkinter.StringVar()
                                )
                            self.tree_edit_UI_Entry_init(
                                obj_root="edit_root",
        ...
        if tmp_type == "自定义":
            ...
            self.UIObject["edit_root_Combobox_platform"]["value"] = tuple(
                tmp_list_platform
            )
            if con_action == "init":
            ...
            tmp_list_model = self.UIData["edit_root_Combobox_dict"][
            ...
            self.UIObject["edit_root_Combobox_Server_auto"]["value"] = tuple(
                tmp_list_server_auto
            )
            tmp_list_server_type = self.UIData["edit_root_Combobox_Server_type_list"]
            self.UIObject["edit_root_Combobox_Server_type"]["value"] = tuple(
                tmp_list_server_type
            )
            if con_action == "init":
    ...
    def tree_edit_UI_Entry_update(self, action, con_action):
        if con_action == "init":
            if action == "update":
                if self.hash_key in self.UIData["Account_data"]:
                    self.UIData["edit_root_Entry_ID_StringVar"].set(
                        str(self.UIData["Account_data"][self.hash_key].id)
                    )
                    ...
                    self.UIData["edit_root_Entry_Server_access_token_StringVar"].set(
                        self.UIData["Account_data"][
                            self.hash_key
                        ].post_info.access_token
                    )
                    type_this = self.get_type_name(
                        str(
                            self.UIData["Account_data"][self.hash_key].platform[
                                "platform"
                            ]
                        ),
                        str(self.UIData["Account_data"][self.hash_key].platform["sdk"]),
                        str(
                            self.UIData["Account_data"][self.hash_key].platform["model"]
                        ),
                    ...
                    if (
                        ...
                        if (
                            ...
                            tmp_i = 0
                            for tmp_data_this in self.UIData["Account_data"][
                                self.hash_key
                            ].extends["qsign-server"]:
                                key_pare = [
                                    "edit_root_Entry_qsign_addr_%d" % tmp_i,
                                    "edit_root_Entry_qsign_key_%d" % tmp_i,
                                ]
                                if (
                                    ...
                                    self.UIData[key_pare[0] + "_StringVar"].set(
                                        tmp_data_this["addr"]
                                    )
                                    self.UIData[key_pare[1] + "_StringVar"] = (
                                        tkinter.StringVar()
                                    )
                                    self.UIData[key_pare[1] + "_StringVar"].set(
                                        tmp_data_this["key"]
                                    )
                                    tmp_i += 1
                                else:
    ...
    def get_type_name(
        self, platform_platform, platform_sdk, platform_model, server_auto, server_type
    ):
        res = None
        list_data_check = [
            platform_platform,
