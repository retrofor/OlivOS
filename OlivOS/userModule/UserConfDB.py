...
def get_group_hash(
    platform: "str", group_id: "str|int", host_id: "str|int|None" = None, *args
):
...
class DataBaseAPI:
    ...
    def __init_thread(self):
        ...
        conn = sqlite3.connect(
            database=DATABASE_PATH, timeout=self.timeout, check_same_thread=False
        )
    ...
    def _init_database(self):
        ...
        if svn != DATABASE_SVN:
            if svn == 0:
                ...
                self.proc_log(
                    3,
                    "用户自定义数据库版本不符合，数据库版本为{0}，OlivOS中所需版本为{1}".format(
                        svn, DATABASE_SVN
                    ),
                )
    ...
    def _init_namespace(self, namespace=None):
        ...
        sql_list = [
            DATABASE_SQL["create.table.namespace"].format(
                namespace_hash=namespace_hash
            ),
            DATABASE_SQL["create.trigger.namespace"].format(
                namespace_hash=namespace_hash
            ),
            (
                DATABASE_SQL["insert.table_master"],
                {"hash_namespace": namespace_hash, "str_namespace": namespace},
            ),
        ]
    ...
    def get_config(
        ...
        sql_this = DATABASE_SQL["select.namespace.conf"].format(
            namespace_hash=namespace_hashed
        )
        param = {
    ...
    def set_config(
        ...
        sql_this = DATABASE_SQL["insert.namespace"].format(
            namespace_hash=namespace_hashed
        )
        param = {
            "hash_key_basic": basic_hashed,
    ...
    def get_basic_config(
        self, namespace: "str|None", key: "str", default_value=None, pkl=False
    ):
        """
    ...
    def set_user_config(
        ...
        return self.set_config(
            namespace=namespace, key=key, value=value, basic_hashed=basic_hash, pkl=pkl
        )
    ...
    def set_group_config_from_event(
        self, namespace: "str|None", key: "str", value, plugin_event, pkl=False
    ):
        """
        设置消息事件对应群组的配置项
    ...
    def set_user_config_from_event(
        self, namespace: "str|None", key: "str", value, plugin_event, pkl=False
    ):
        """
        设置消息事件对应用户的配置项
